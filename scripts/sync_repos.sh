...
#
# Sync directories in the main TinySwift repository into dedicated child
...
GIT_USERNAME="TinySwiftInfraBot"
git config --global user.email "tinyswift-external-infra@google.com"
git config --global user.name "$GIT_USERNAME"

declare -A MIRRORS
MIRRORS["utils/vim"]="vim-tinyswift-lang"

for dir in "${!MIRRORS[@]}"; do
  SRC_DIR="$dir"
  DEST_REPO="${MIRRORS[$SRC_DIR]}"
  DEST_REPO_URL="https://$GIT_USERNAME:$API_TOKEN_GITHUB@github.com/tinyswift-language/$DEST_REPO.git"
  ...
  if ! git diff --cached --quiet; then
    ...
    git commit -F- <<EOF
Sync $DEST_REPO to tinyswift-language/tinyswift-lang@$COMMIT_SHA
