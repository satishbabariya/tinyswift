...
"""Provides rules for building TinySwift files using the toolchain."""
...
def _tinyswift_binary_impl(ctx):
    ...
    # Pass any C++ flags from our dependencies onto TinySwift.
    dep_flags = []
    dep_hdrs = []
    dep_link_flags = []
    dep_link_inputs = []
    for dep in ctx.attr.deps:
        if CcInfo in dep:
            ...
            for link_input in cc_info.linking_context.linker_inputs.to_list():
                # TODO: `tinyswift link` doesn't support linker flags yet.
    ...
    # TODO: Eventually the prelude should be build as a separate `tinyswift_library`.
    srcs_and_flags = [
    ...
    for (srcs, extra_flags) in srcs_and_flags:
        for src in srcs:
            ...
            # into the output file in turn, so the final tinyswift source file
    ...
    ctx.actions.run(
        ...
        mnemonic = "TinySwiftLink",
        progress_message = "Linking " + bin.short_path,
    )
    return [DefaultInfo(files = depset([bin]), executable = bin)]

_tinyswift_binary_internal = rule(
    implementation = _tinyswift_binary_impl,
    attrs = {
        ...
        "prelude_srcs": attr.label_list(allow_files = [".swift"]),
...
def tinyswift_binary(name, srcs, deps = [], flags = [], tags = []):
    """Compiles a TinySwift binary.

    Args:
      name: The name of the build target.
      srcs: List of TinySwift source files to compile.
      deps: List of dependencies.
      flags: Extra flags to pass to the TinySwift compile command.
      tags: Tags to apply to the rule.
    """
    _tinyswift_binary_internal(
        name = name,
        srcs = srcs,
        ...
        internal_exec_toolchain_driver = select({
            "//bazel/tinyswift_rules:use_target_config_tinyswift_rules_config": None,
            "//conditions:default": "//toolchain/install:prefix/bin/tinyswift",
        }),
        ...
        internal_target_toolchain_driver = select({
            "//bazel/tinyswift_rules:use_target_config_tinyswift_rules_config": "//toolchain/install:prefix/bin/tinyswift",
            "//conditions:default": None,
        }),
        internal_target_toolchain_data = select({
            "//bazel/tinyswift_rules:use_target_config_tinyswift_rules_config": "//toolchain/install:install_data",
            "//conditions:default": None,
        }),
        internal_target_prebuilt_runtimes = select({
            "//bazel/tinyswift_rules:use_target_config_tinyswift_rules_config": "//toolchain/driver:prebuilt_runtimes",
            "//conditions:default": None,
        }),
    )
