...
"""Check that non-test C++ rules only depend on TinySwift and LLVM.

TinySwift works to ensure its user-visible libraries and binaries only depend on
...
include TinySwift and LLVM code.
"""
...
for dep in deps:
    ...
    if repo == "@@+llvm_project+llvm-project":
        ...
        # in TinySwift. We can expand the above list if use cases emerge.
        if package not in (
            "clang",
    ...
    # TinySwift code is always allowed.
    if repo == "" and not rule.startswith("third_party"):
        continue

    # LLVM code managed in the TinySwift repository is still LLVM code and OK.
