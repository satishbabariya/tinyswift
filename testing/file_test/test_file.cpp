...
static auto AutoFillDidOpenParams(llvm::json::Object& params,
  ...
  TINYSWIFT_ASSIGN_OR_RETURN(auto file_path, ExtractFilePathFromUri(*uri));
...
static auto ReplaceLspKeywordAt(std::string& content, size_t keyword_pos,
  ...
  if (!extra_content.empty()) {
    ...
    TINYSWIFT_CHECK(parsed_extra_content.kind() == llvm::json::Value::Object);
    if (extra_content_label == "params" &&
        method_or_id == "textDocument/didOpen") {
      TINYSWIFT_RETURN_IF_ERROR(
...
static auto TransformExpectation(int line_index, llvm::StringRef in)
  ...
  while (keyword_pos != std::string::npos) {
    ...
    TINYSWIFT_CHECK(line_keyword_cursor.consume_front("[["));
...
static auto TryConsumeCheck(
  ...
  if (!running_autoupdate) {
    ...
    TINYSWIFT_ASSIGN_OR_RETURN(Matcher<std::string> check_matcher,
...
static auto ProcessFileContent(llvm::StringRef filename,
  ...
  while (!content_cursor.empty()) {
    ...
    TINYSWIFT_ASSIGN_OR_RETURN(is_consumed,
  ...
  TINYSWIFT_RETURN_IF_ERROR(FinishSplit(filename, /*is_include_file=*/!test_file,
                                     split_state, splits));
...
auto ProcessTestFile(llvm::StringRef test_name, bool running_autoupdate)
  ...
  TINYSWIFT_ASSIGN_OR_RETURN(test_file.input_content, ReadFile(test_name.str()));
  ...
  TINYSWIFT_RETURN_IF_ERROR(ProcessFileContent(
  ...
  for (size_t i = 0; i < include_files.size(); ++i) {
    ...
    TINYSWIFT_ASSIGN_OR_RETURN(std::string content, ReadFile(filename));
