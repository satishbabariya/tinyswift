...
auto FileTestCase::TestBody() -> void {
  ...
  TINYSWIFT_CHECK(test_info_->test_result,
...
// Implements main() within the TinySwift::Testing namespace for convenience.
static auto Main(int argc, char** argv) -> ErrorOr<int> {
  ...
  TinySwift::InitLLVM init_llvm(argc, argv);
  ...
  TINYSWIFT_RETURN_IF_ERROR(RegisterTests(&test_factory, exe_path, tests));
...
auto main(int argc, char** argv) -> int {
  if (auto result = TinySwift::Testing::Main(argc, argv); result.ok()) {
    return *result;
  } else {
