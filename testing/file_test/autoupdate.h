...
#ifndef TINYSWIFT_TESTING_FILE_TEST_AUTOUPDATE_H_
#define TINYSWIFT_TESTING_FILE_TEST_AUTOUPDATE_H_
 ...
 public:
  ...
  explicit FileTestAutoupdater(
    ...
    for (const auto& replacement : line_number_replacements_) {
      TINYSWIFT_CHECK(replacement.has_file || default_file_re_,
                   "For replacement with pattern `{0}` to have has_file=false, "
                   "override GetDefaultFileRE.",
                   replacement.re->pattern());
      TINYSWIFT_CHECK(replacement.re->ok(), "Invalid line replacement RE2: {0}",
...
#endif  // TINYSWIFT_TESTING_FILE_TEST_AUTOUPDATE_H_
