...
#ifndef TINYSWIFT_TESTING_BASE_SOURCE_GEN_H_
#define TINYSWIFT_TESTING_BASE_SOURCE_GEN_H_
...
// - Generate code that includes most TinySwift language features (and whatever
...
//
// Note that the default and primary generation target is interesting TinySwift
// source code. We have a best-effort to alternatively generate comparable C++
// constructs to the TinySwift ones for comparative benchmarking, but there is no
...
class SourceGen {
 public:
  ...
  struct TypeUseParams {
    ...
    // with a TinySwift and C++ string, and their associated weight.
    struct FixedTypeWeight {
      llvm::StringRef tinyswift_spelling;
    ...
    llvm::SmallVector<FixedTypeWeight> fixed_type_weights = {
        ...
        {.swift_spelling = "bool", .cpp_spelling = "bool", .weight = 25},
        {.swift_spelling = "i32", .cpp_spelling = "int", .weight = 20},
        {.swift_spelling = "i64",
         .cpp_spelling = "std::int64_t",
         .weight = 10},
        {.swift_spelling = "i32*", .cpp_spelling = "int*", .weight = 5},
        {.swift_spelling = "i64*",
         .cpp_spelling = "std::int64_t*",
         .weight = 5},

        // A weight of 5 distributed across tuple structures
        {.swift_spelling = "(bool, i64)",
         .cpp_spelling = "std::pair<bool, std::int64_t>",
         .weight = 2},
        {.swift_spelling = "(i32, i64*)",
  ...
  // Access a global instance of this type to generate TinySwift code for
  ...
  // Construct a source generator for the provided language, by default TinySwift.
  explicit SourceGen(Language language = Language::TinySwift);
  ...
  // of constructs as TinySwift would for the given line count over keeping the
  ...
  // include [0-9_]. Both TinySwift and C++ keywords are excluded along with any
  ...
  // of them in TinySwift source code. The maximum length in this default
...
#endif  // TINYSWIFT_TESTING_BASE_SOURCE_GEN_H_
