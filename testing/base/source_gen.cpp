...
auto SourceGen::ClassGenState::GetValidTypeName() -> llvm::StringRef {
  ...
  for (;;) {
    ...
    TINYSWIFT_CHECK(last_type_name_index_ != initial_last_type_name_index,
...
auto SourceGen::ClassGenState::BuildClassAndTypeNames(
  ...
  for (const auto& fixed_type_weight : type_use_params.fixed_type_weights) {
    ...
    valid_type_names_.Insert(gen.IsCpp() ? fixed_type_weight.cpp_spelling
                                         : fixed_type_weight.swift_spelling);
  }
  ...
  TINYSWIFT_CHECK(static_cast<int>(type_names_.size()) == num_declared_types);
  ...
  TINYSWIFT_CHECK(static_cast<int>(type_names_.size()) == num_types);
...
// are loosely based on what we expect to make TinySwift code readable, and might
...
auto SourceGen::GenApiFileDenseDecls(int target_lines,
  ...
  TINYSWIFT_CHECK(target_lines > NumFileCommentLines + avg_class_lines,
  ...
  source << "// Generated " << (!IsCpp() ? "TinySwift" : "C++")
         << " source file.\n";
  source << llvm::formatv(
  ...
  // TinySwift uses an implicitly imported prelude to get builtin types, but C++
  ...
  TINYSWIFT_CHECK(class_gen_state.public_function_param_counts().empty());
  TINYSWIFT_CHECK(class_gen_state.public_method_param_counts().empty());
  TINYSWIFT_CHECK(class_gen_state.private_function_param_counts().empty());
  TINYSWIFT_CHECK(class_gen_state.private_method_param_counts().empty());
  TINYSWIFT_CHECK(class_gen_state.class_names().empty());
  TINYSWIFT_CHECK(class_gen_state.type_names().empty());
...
auto SourceGen::GetSingleLengthIdentifiers(int length, int number)
  ...
  if (static_cast<int>(idents.size()) < number) {
    ...
    TINYSWIFT_CHECK(static_cast<int>(idents.size()) == number);
  }
...
constexpr static llvm::StringRef NonTinySwiftCppKeywords[] = {
...
// keywords both in TinySwift and C++.
//
...
auto SourceGen::GenerateRandomIdentifier(
  ...
  do {
    ...
    for (int i : llvm::seq<int>(1, dest_storage.size())) {
      ...
      llvm::is_contained(NonTinySwiftCppKeywords, ident) ||
...
auto SourceGen::GetIdentifiersImpl(int number, int min_length, int max_length,
    ...
    -> llvm::SmallVector<llvm::StringRef> {
  TINYSWIFT_CHECK(min_length <= max_length);
  TINYSWIFT_CHECK(
  ...
  TINYSWIFT_CHECK(number_rem == 0, "Unexpected number remaining: {0}", number_rem);
  TINYSWIFT_CHECK(static_cast<int>(idents.size()) == number,
...
auto SourceGen::GetShuffledInts(int number, int min, int max)
  ...
  TINYSWIFT_CHECK(static_cast<int>(ints.size()) == number);
 ...
 public:
  ...
  auto Pop() -> llvm::StringRef {
    for (auto end = data_->rend(); it_ != end; ++it_) {
      ...
      TINYSWIFT_CHECK(insert.key() == data_->back());
