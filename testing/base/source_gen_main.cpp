...
constexpr CommandLine::CommandInfo Info = {
    .name = "source_gen",
    .help = R"""(
A source generator for TinySwift.
)""",
};
...
primary language focus is generating TinySwift.
)""",
};
  ...
  auto parse_result = CommandLine::Parse(
      args, llvm::outs(), Info, [&](CommandLine::CommandBuilder& b) {
        ...
        b.AddOneOfOption(LanguageArgInfo, [&](auto& arg_b) {
          arg_b.SetOneOf(
              {
                  arg_b.OneOfValue("tinyswift", SourceGen::Language::TinySwift)
                      .Default(true),
...
auto main(int argc, char** argv) -> int {
  ...
  TinySwift::SetWorkingDirForBazelRun();

  llvm::SmallVector<llvm::StringRef> args(argv + 1, argv + argc);
  bool success = TinySwift::Testing::Run(args);
