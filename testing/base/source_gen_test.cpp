...
auto TestCompile(llvm::StringRef source) -> bool {
  ...
  fs->addFile("test.swift", /*ModificationTime=*/0,
              llvm::MemoryBuffer::getMemBuffer(source));
  return driver.RunCommand({"compile", "--phase=check", "test.swift"}).success;
}
...
TEST(SourceGenTest, GenApiFileDenseDeclsTest) {
  ...
  // Make sure we generated valid TinySwift code.
...
TEST(SourceGenTest, GenApiFileDenseDeclsCppTest) {
  ...
  // TODO: When the driver supports compiling C++ code as easily as TinySwift, we
