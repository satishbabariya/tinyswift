...
#ifndef TINYSWIFT_TOOLCHAIN_BASE_INSTALL_PATHS_H_
#define TINYSWIFT_TOOLCHAIN_BASE_INSTALL_PATHS_H_
...
//
// The TinySwift toolchain expects to be installed into a tree rooted at `root_`.
...
// `<some prefix>/lib/tinyswift`, with symlinks in the other parts of the FHS-based
...
// out what TinySwift's should be within a Windows prefix and how much of the
...
class InstallPaths {
 public:
  ...
  // for internal development purposes or other Bazel users of the TinySwift
  // library.
  //
  // This method of construction also ensures the result is valid. If detection
  // fails for any reason, it will `TINYSWIFT_CHECK` fail with the error message.
  ...
  // using TinySwift in an environment with an unusual path to the installed files.
 ...
 private:
  ...
  // `root()/lib/tinyswift/tinyswift_install.txt". If not, calls `SetError` with the
  ...
  // this will look like:
  // `bazel-bin/some/bazel/target.runfiles/_main/toolchain/install/prefix/lib/tinyswift`
  //
  // When installed, it's expected to be similar to the CMake install prefix,
  // followed by `lib/tinyswift`:
  //
  // - `/usr/lib/tinyswift` or `/usr/local/lib/tinyswift` on Linux and most BSDs.
  // - `/opt/homebrew/lib/tinyswift` or similar on macOS with Homebrew.
  // - TODO: Figure out if this is `C:/Program Files/TinySwift` or something else
  //   on Windows.
  //
  ...
  // BUILD's `install_dirs` entry for `lib/tinyswift`.
...
#endif  // TINYSWIFT_TOOLCHAIN_BASE_INSTALL_PATHS_H_
