...
#ifndef TINYSWIFT_TOOLCHAIN_BASE_VALUE_IDS_H_
#define TINYSWIFT_TOOLCHAIN_BASE_VALUE_IDS_H_
...
struct PackageNameId : public IdBase<PackageNameId> {
  ...
  auto AsSpecialName() const -> llvm::StringLiteral {
    TINYSWIFT_CHECK(index <= NoneIndex);
    if (*this == None) {
      return "Main";
    }
    if (*this == Core) {
    ...
    TINYSWIFT_FATAL("Unknown special package name kind {0}", index);
  }
...
#endif  // TINYSWIFT_TOOLCHAIN_BASE_VALUE_IDS_H_
