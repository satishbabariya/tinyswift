...
// - `TINYSWIFT_LLVM_TOOL` is available for every tool.
//   - `TINYSWIFT_LLVM_MAIN_TOOL` is available for each tool with a distinct
//     `MainFn` symbol name.
//   - `TINYSWIFT_LLVM_ALIAS_TOOL` is available for each tool that is an alias of
//     some other tool. It's `MainFn` will be the alias-target symbol name.
//
// See toolchain/driver/llvm_tools.bzl for more details.

#ifndef TINYSWIFT_LLVM_TOOL
#define TINYSWIFT_LLVM_TOOL(Id, Name, BinName, MainFn)
#endif

#ifndef TINYSWIFT_LLVM_MAIN_TOOL
#define TINYSWIFT_LLVM_MAIN_TOOL(Id, Name, BinName, MainFn) \\
  TINYSWIFT_LLVM_TOOL(Id, Name, BinName, MainFn)
#endif

#ifndef TINYSWIFT_LLVM_ALIAS_TOOL
#define TINYSWIFT_LLVM_ALIAS_TOOL(Id, Name, BinName, MainFn) \\
  TINYSWIFT_LLVM_TOOL(Id, Name, BinName, MainFn)
#endif

{}

#undef TINYSWIFT_LLVM_TOOL
#undef TINYSWIFT_LLVM_MAIN_TOOL
#undef TINYSWIFT_LLVM_ALIAS_TOOL
"""

_DEF_MACRO_TEMPLATE = """
TINYSWIFT_LLVM_{kind}TOOL({id}, "{name}", "{bin_name}", {main_fn})
""".strip()
