...
class InstallPathsTest : public ::testing::Test {
 public:
  InstallPathsTest() {
    ...
    TINYSWIFT_CHECK(test_runfiles_ != nullptr, "{0}", error);
  }
  ...
  auto TestInstallPaths(const InstallPaths& paths) -> void {
    ...
    EXPECT_THAT(root_path.native(), EndsWith("lib/tinyswift/"));
    EXPECT_THAT(
        root.Access("../../bin/tinyswift", Filesystem::AccessCheckFlags::Execute),
        IsSuccess(Eq(true)))
        << "path: " << (root_path / "../../bin/tinyswift");

    std::filesystem::path core_package_path = paths.core_package();
    ASSERT_THAT(core_package_path, StartsWith(root_path));
    EXPECT_THAT(Filesystem::Cwd().Access(core_package_path / "prelude.swift"),
                IsSuccess(Eq(true)))
...
TEST_F(InstallPathsTest, RootBusybox) {
  std::string installed_busybox_path = test_runfiles_->Rlocation(
      "tinyswift/toolchain/install/prefix/lib/tinyswift/tinyswift-busybox");
...
TEST_F(InstallPathsTest, RootExplicit) {
  std::string marker_path = test_runfiles_->Rlocation(
      "tinyswift/toolchain/install/prefix/lib/tinyswift/tinyswift_install.txt");

  llvm::StringRef root_path = marker_path;
  TINYSWIFT_CHECK(root_path.consume_back("tinyswift_install.txt"),
...
TEST_F(InstallPathsTest, BinaryRunfiles) {
  std::filesystem::path test_binary_path =
      test_runfiles_->Rlocation("tinyswift/toolchain/base/test_binary");
