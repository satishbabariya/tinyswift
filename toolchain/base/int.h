...
#ifndef TINYSWIFT_TOOLCHAIN_BASE_INT_H_
#define TINYSWIFT_TOOLCHAIN_BASE_INT_H_
...
class IntId : public Printable<IntId> {
 public:
  ...
  constexpr auto AsValue() const -> int {
    TINYSWIFT_DCHECK(is_embedded_value());
    return id_;
  }
  ...
  constexpr auto AsTokenPayload() const -> uint32_t {
    ...
    TINYSWIFT_DCHECK(*this == MakeFromTokenPayload(payload));
    return payload;
  }
...
class IntStore {
 ...
 private:
  ...
  static auto MakeIndexOrNone(int index) -> IntId {
    TINYSWIFT_DCHECK(index >= 0 && index <= IntId::NoneIndex);
...
#endif  // TINYSWIFT_TOOLCHAIN_BASE_INT_H_
