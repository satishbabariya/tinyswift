...
#ifndef TINYSWIFT_TOOLCHAIN_BASE_VALUE_STORE_H_
#define TINYSWIFT_TOOLCHAIN_BASE_VALUE_STORE_H_
...
class ValueStore
 ...
 public:
  ...
  auto Add(ValueType value) -> IdType {
    ...
    TINYSWIFT_DCHECK(size_ < std::numeric_limits<int32_t>::max(), "Id overflow");

    IdType id = tag_.Apply(size_);
    auto [chunk_index, pos] = RawIndexToChunkIndices(size_);
    ++size_;

    TINYSWIFT_DCHECK(static_cast<size_t>(chunk_index) <= chunks_.size(),
                  "{0} <= {1}", chunk_index, chunks_.size());
    if (static_cast<size_t>(chunk_index) == chunks_.size()) {
      chunks_.emplace_back();
    }

    TINYSWIFT_DCHECK(pos == chunks_[chunk_index].size());
  ...
  auto Get(IdType id) -> RefType {
    TINYSWIFT_DCHECK(id.index >= 0, "{0}", id);
  ...
  auto GetRawIndex(IdT id) const -> int32_t {
    TINYSWIFT_DCHECK(id.index >= 0, "{0}", index);
    auto index = tag_.Remove(id);
#ifndef NDEBUG
    if (index >= size_) {
  ...
  struct Chunk {
   public:
    ...
    auto Add(ValueType&& value) -> void {
      TINYSWIFT_DCHECK(num_ < Capacity());
    ...
    auto UninitializedFill(int32_t fill_count, ConstRefType default_value)
        -> void {
      TINYSWIFT_DCHECK(num_ + fill_count <= Capacity());
...
#endif  // TINYSWIFT_TOOLCHAIN_BASE_VALUE_STORE_H_
