...
auto TokenizedBuffer::GetMatchedClosingToken(TokenIndex opening_token) const
  ...
  TINYSWIFT_CHECK(opening_token_info.kind().is_opening_symbol(), "{0}",
...
auto TokenizedBuffer::GetMatchedOpeningToken(TokenIndex closing_token) const
  ...
  TINYSWIFT_CHECK(closing_token_info.kind().is_closing_symbol(), "{0}",
...
static auto ComputeDecimalPrintedWidth(int number) -> int {
  TINYSWIFT_CHECK(number >= 0, "Negative numbers are not supported.");
  if (number == 0) {
    return 1;
  }
...
auto TokenizedBuffer::FindLineIndex(int32_t byte_offset) const -> LineIndex {
  TINYSWIFT_DCHECK(line_infos_.size() > 0);
  ...
  TINYSWIFT_DCHECK(line_it->start <= byte_offset);
...
auto TokenizedBuffer::SourcePointerToDiagnosticLoc(const char* loc) const
  ...
  TINYSWIFT_CHECK(StringRefContainsPointer(source_->text(), loc),
  ...
  TINYSWIFT_CHECK(next_line_it != line_range.begin(),
...
auto TokenizedBuffer::OverlapsWithDumpSemIRRange(
  ...
  TINYSWIFT_CHECK(!dump_sem_ir_ranges_.empty());
