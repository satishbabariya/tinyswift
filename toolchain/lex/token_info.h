...
#ifndef TINYSWIFT_TOOLCHAIN_LEX_TOKEN_INFO_H_
#define TINYSWIFT_TOOLCHAIN_LEX_TOKEN_INFO_H_
...
class TokenInfo {
 public:
  ...
  auto closing_token_index() const -> TokenIndex {
    TINYSWIFT_DCHECK(kind().is_opening_symbol());
  ...
  auto opening_token_index() const -> TokenIndex {
    TINYSWIFT_DCHECK(kind().is_closing_symbol());
    return TokenIndex(token_payload_);
  }
  ...
  auto error_length() const -> int {
    TINYSWIFT_DCHECK(kind() == TokenKind::Error);
...
#endif  // TINYSWIFT_TOOLCHAIN_LEX_TOKEN_INFO_H_
