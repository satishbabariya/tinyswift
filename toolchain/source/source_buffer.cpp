...
auto SourceBuffer::MakeFromFile(llvm::vfs::FileSystem& fs,
  ...
  if (file.getError()) {
    TINYSWIFT_DIAGNOSTIC(ErrorOpeningFile, Error,
  ...
  if (status.getError()) {
    TINYSWIFT_DIAGNOSTIC(ErrorStattingFile, Error, "error statting file: {0}",
                      std::string);
...
auto SourceBuffer::MakeFromMemoryBuffer(
  ...
  if (buffer.getError()) {
    TINYSWIFT_DIAGNOSTIC(ErrorReadingFile, Error, "error reading file: {0}",
                      std::string);
  ...
  if (buffer.get()->getBufferSize() >= std::numeric_limits<int32_t>::max()) {
    TINYSWIFT_DIAGNOSTIC(FileTooLarge, Error,
