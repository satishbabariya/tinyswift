...
Validates that each diagnostic declared with TINYSWIFT_DIAGNOSTIC_KIND is
referenced by one (and only one) TINYSWIFT_DIAGNOSTIC.
"""
...
def load_diagnostic_kind() -> Set[str]:
    ...
    decls = set(re.findall(r"^\s+TINYSWIFT_DIAGNOSTIC_KIND\((.+)\)", content))
...
def load_diagnostic_uses_in(
    ...
    """Returns the path's TINYSWIFT_DIAGNOSTIC uses."""
    content = path.read_text()
    ...
    for m in re.finditer(r"TINYSWIFT_DIAGNOSTIC\(\s*(\w+),", content):
        diag = m.group(1)
        if diag in IGNORED:
            continue
        line += content.count("\n", line_offset, m.start())
...
def load_diagnostic_uses() -> Dict[str, List[Loc]]:
    """Returns all TINYSWIFT_DIAGNOSTIC uses."""
