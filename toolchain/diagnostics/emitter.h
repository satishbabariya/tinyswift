...
#ifndef TINYSWIFT_TOOLCHAIN_DIAGNOSTICS_EMITTER_H_
#define TINYSWIFT_TOOLCHAIN_DIAGNOSTICS_EMITTER_H_
...
// `TINYSWIFT_NO_LOC_DIAGNOSTIC` macro, or some other factoring. But it might end
...
auto Emitter<LocT>::Builder::Note(
  ...
  TINYSWIFT_CHECK(diagnostic_base.Level == Level::Note ||
...
auto Emitter<LocT>::Builder::FormatFn(const Message& message,
  ...
  TINYSWIFT_CHECK(message.format_args.size() == sizeof...(Args),
...
auto Emitter<LocT>::MakeAny(Arg arg) -> llvm::Any {
  ...
  TINYSWIFT_CHECK(llvm::any_cast<Storage>(&converted),
...
#endif  // TINYSWIFT_TOOLCHAIN_DIAGNOSTICS_EMITTER_H_
