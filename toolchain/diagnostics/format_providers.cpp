...
auto llvm::format_provider<TinySwift::Diagnostics::BoolAsSelect>::format(
    const TinySwift::Diagnostics::BoolAsSelect& wrapper, raw_ostream& out,
  ...
  TINYSWIFT_CHECK(
      ...
      style);
  TINYSWIFT_CHECK(style.find('|', sep + 1) == llvm::StringRef::npos,
...
auto llvm::format_provider<TinySwift::Diagnostics::IntAsSelect>::format(
    const TinySwift::Diagnostics::IntAsSelect& wrapper, raw_ostream& out,
    StringRef style) -> void {
  if (style == "s") {
    if (wrapper.value != 1) {
      out << "s";
  ...
  while (!cursor.empty()) {
    ...
    TINYSWIFT_CHECK(pair_sep != llvm::StringRef::npos,
    ...
    if (comp.empty()) {
      ...
      int value;
      TINYSWIFT_CHECK(to_integer(comp, value),
      ...
      auto result = llvm::StringSwitch<std::optional<bool>>(op)
                        ...
                        .Default(std::nullopt);
      if (!result) {
        TINYSWIFT_FATAL("IntAsSelect has unrecognized comparison: `{0}`", style);
      }
      if (*result) {
        out << output_string;
        return;
      ...
      TINYSWIFT_FATAL("IntAsSelect has unrecognized syntax: `{0}`", style);
    }
  }

  TINYSWIFT_FATAL("IntAsSelect doesn't handle `{0}`: `{1}`", wrapper.value, style);
}
