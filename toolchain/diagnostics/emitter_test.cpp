...
TEST_F(EmitterTest, EmitSimpleError) {
  TINYSWIFT_DIAGNOSTIC(TestDiagnostic, Error, "simple error");
...
TEST_F(EmitterTest, EmitSimpleWarning) {
  TINYSWIFT_DIAGNOSTIC(TestDiagnostic, Warning, "simple warning");
...
TEST_F(EmitterTest, EmitOneArgDiagnostic) {
  TINYSWIFT_DIAGNOSTIC(TestDiagnostic, Error, "arg: `{0}`", std::string);
  EXPECT_CALL(consumer_, HandleDiagnostic(IsSingleDiagnostic(
                             Diagnostics::Kind::TestDiagnostic,
...
TEST_F(EmitterTest, EmitNote) {
  TINYSWIFT_DIAGNOSTIC(TestDiagnostic, Warning, "simple warning");
  TINYSWIFT_DIAGNOSTIC(TestDiagnosticNote, Note, "note");
  EXPECT_CALL(
      consumer_,
