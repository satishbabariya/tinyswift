...
#ifndef TINYSWIFT_TOOLCHAIN_DRIVER_DRIVER_FILE_TEST_BASE_H_
#define TINYSWIFT_TOOLCHAIN_DRIVER_DRIVER_FILE_TEST_BASE_H_
...
static auto GetSharedTestData(llvm::StringRef exe_path)
  ...
  static ErrorOr<SharedTestData> data = [&]() -> ErrorOr<SharedTestData> {
    ...
    TINYSWIFT_ASSIGN_OR_RETURN(data.prelude_files,
                            data.installation.ReadPreludeManifest());
    for (const auto& file : data.prelude_files) {
      TINYSWIFT_RETURN_IF_ERROR(AddFile(*data.file_system, file));
    }

    llvm::SmallVector<std::string> clang_header_files;
    TINYSWIFT_ASSIGN_OR_RETURN(clang_header_files,
  ...
  TINYSWIFT_CHECK(data.ok(), "{0}", data.error());
  return &*data;
}
...
TINYSWIFT_FILE_TEST_FACTORY(ToolchainFileTest)
...
auto ToolchainFileTest::GetDefaultArgs() const
  ...
  if (component_ == "lex") {
    ...
    TINYSWIFT_FATAL("Unexpected test component {0}: {1}", component_, test_name());
  }
...
#endif  // TINYSWIFT_TOOLCHAIN_DRIVER_DRIVER_FILE_TEST_BASE_H_
