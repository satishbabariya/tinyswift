...
#ifndef TINYSWIFT_TOOLCHAIN_SEM_IR_IDS_H_
#define TINYSWIFT_TOOLCHAIN_SEM_IR_IDS_H_
...
struct BoolValue : public IdBase<BoolValue> {
  ...
  constexpr auto ToBool() -> bool {
    TINYSWIFT_CHECK(*this == False || *this == True, "Invalid bool value {0}",
                 index);
    return *this != False;
  }
...
struct FloatKind : public IdBase<FloatKind> {
  ...
  // Supported IEEE-754 interchange formats. These correspond to TinySwift `fN`
  // type literal syntax.
  ...
  // target-dependent.
  // TODO: Add a mechanism to use these types from TinySwift code.
...
//
//   #define TINYSWIFT_SPECIAL_NAME_ID_FOR_XYZ(Name) ...
//   TINYSWIFT_SPECIAL_NAME_ID(TINYSWIFT_SPECIAL_NAME_ID_FOR_XYZ)
//   #undef TINYSWIFT_SPECIAL_NAME_ID_FOR_XYZ
#define TINYSWIFT_SPECIAL_NAME_ID(X)                                \
  /* The name of `base`. */                                      \
  X(Base)                                                        \
...
#define TINYSWIFT_SPECIAL_NAME_ID_FOR_ENUM(Name) Name,
    TINYSWIFT_SPECIAL_NAME_ID(TINYSWIFT_SPECIAL_NAME_ID_FOR_ENUM)
#undef TINYSWIFT_SPECIAL_NAME_ID_FOR_ENUM
  };

  // For each SpecialNameId, provide a matching `NameId` instance for
  // convenience.
#define TINYSWIFT_SPECIAL_NAME_ID_FOR_DECL(Name) static const NameId Name;
  TINYSWIFT_SPECIAL_NAME_ID(TINYSWIFT_SPECIAL_NAME_ID_FOR_DECL)
#undef TINYSWIFT_SPECIAL_NAME_ID_FOR_DECL
...
#define TINYSWIFT_SPECIAL_NAME_ID_FOR_DEF(Name) \
  inline constexpr NameId NameId::Name =     \
      NameId(NoneIndex - 1 - static_cast<int>(NameId::SpecialNameId::Name));
TINYSWIFT_SPECIAL_NAME_ID(TINYSWIFT_SPECIAL_NAME_ID_FOR_DEF)
#undef TINYSWIFT_SPECIAL_NAME_ID_FOR_DEF

// Count non-index values, including `None` and special names.
#define TINYSWIFT_SPECIAL_NAME_ID_FOR_COUNT(...) +1
inline constexpr int NameId::NonIndexValueCount =
    1 TINYSWIFT_SPECIAL_NAME_ID(TINYSWIFT_SPECIAL_NAME_ID_FOR_COUNT);
#undef TINYSWIFT_SPECIAL_NAME_ID_FOR_COUNT
...
class InstBlockIdOrError {
 public:
  ...
  auto inst_block_id() const -> InstBlockId {
    TINYSWIFT_CHECK(!has_error_value());
...
struct LibraryNameId : public IdBase<LibraryNameId> {
  ...
  auto AsStringLiteralValueId() const -> StringLiteralValueId {
    TINYSWIFT_CHECK(index >= NoneIndex, "{0} must be handled directly", *this);
...
struct ImportIRInstId : public IdBase<ImportIRInstId> {
  ...
  constexpr explicit ImportIRInstId(int32_t index) : IdBase(index) {
    TINYSWIFT_DCHECK(index < Max, "Index out of range: {0}", index);
  }
};
...
struct LocId : public IdBase<LocId> {
  ...
  auto AsDesugared() const -> LocId {
    ...
    TINYSWIFT_CHECK(kind() != Kind::InstId, "Use InstStore::GetDesugaredLocId");
  ...
  auto import_ir_inst_id() const -> ImportIRInstId {
    ...
    TINYSWIFT_CHECK(kind() == Kind::ImportIRInstId, "{0}", index);
  ...
  auto inst_id() const -> InstId {
    TINYSWIFT_CHECK(kind() == Kind::None || kind() == Kind::InstId, "{0}", index);
    return InstId(index);
  }
  ...
  auto node_id() const -> Parse::NodeId {
    ...
    TINYSWIFT_CHECK(kind() == Kind::NodeId, "{0}", index);
...
#endif  // TINYSWIFT_TOOLCHAIN_SEM_IR_IDS_H_
