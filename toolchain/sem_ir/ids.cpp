...
auto BoolValue::Print(llvm::raw_ostream& out) const -> void {
  if (*this == False) {
    ...
    TINYSWIFT_FATAL("Invalid bool value {0}", index);
  }
}
...
auto IntKind::Print(llvm::raw_ostream& out) const -> void {
  if (*this == Unsigned) {
    ...
    TINYSWIFT_FATAL("Invalid int kind value {0}", index);
  }
}
...
auto FloatKind::Semantics() const -> const llvm::fltSemantics& {
  switch (this->index) {
    ...
    default:
      TINYSWIFT_FATAL("Unexpected float kind {0}", *this);
  }
}
...
auto NameId::ForIdentifier(IdentifierId id) -> NameId {
  if (id.index >= 0) {
    ...
    TINYSWIFT_FATAL("Unexpected identifier ID {0}", id);
  }
}
...
auto NameId::ForPackageName(PackageNameId id) -> NameId {
  if (auto identifier_id = id.AsIdentifierId(); identifier_id.has_value()) {
    ...
    TINYSWIFT_FATAL("Unexpected package ID {0}", id);
  }
}
...
auto NameId::Print(llvm::raw_ostream& out) const -> void {
  ...
  TINYSWIFT_CHECK(special_name_id, "Unknown index {0}", index);

  switch (*special_name_id) {
#define TINYSWIFT_SPECIAL_NAME_ID_FOR_PRINT(Name) \
  case SpecialNameId::Name:                    \
    out << #Name;                              \
    break;
    TINYSWIFT_SPECIAL_NAME_ID(TINYSWIFT_SPECIAL_NAME_ID_FOR_PRINT)
#undef TINYSWIFT_SPECIAL_NAME_ID_FOR_PRINT
  }
  out << ")";
}
...
auto LibraryNameId::ForStringLiteralValueId(StringLiteralValueId id)
    -> LibraryNameId {
  TINYSWIFT_CHECK(id.index >= NoneIndex, "Unexpected library name ID {0}", id);
