...
#ifndef TINYSWIFT_TOOLCHAIN_SEM_IR_INST_H_
#define TINYSWIFT_TOOLCHAIN_SEM_IR_INST_H_
 ...
 public:
  ...
  class ArgAndKind {
   public:
    ...
    auto As() const -> IdT {
      TINYSWIFT_DCHECK(kind_ == IdKind::For<IdT>);
      return IdT(value_);
    }
  ...
  static auto MakeSingleton(InstKind kind) -> Inst {
    TINYSWIFT_CHECK(IsSingletonInstKind(kind));
  ...
  auto As() const -> TypedInst {
    using Info = Internal::InstLikeTypeInfo<TypedInst>;
    TINYSWIFT_CHECK(Is<TypedInst>(), "Casting inst {0} to wrong kind {1}", *this,
                 Info::DebugName());
...
class InstStore {
 public:
  ...
  auto GetAsKnownInstId(InstId inst_id) const -> KnownInstId<InstT> {
    TINYSWIFT_CHECK(Is<InstT>(inst_id), "Casting inst {0} to wrong kind {1}",
 ...
 private:
  ...
  auto GetNonCanonicalLocId(InstId inst_id) const -> LocId {
    auto index = values_.GetRawIndex(inst_id);
    TINYSWIFT_CHECK(static_cast<size_t>(index) < loc_ids_.size(), "{0} {1}", index,
                 loc_ids_.size());
...
class InstBlockStore
 ...
 public:
  ...
  explicit InstBlockStore(llvm::BumpPtrAllocator& allocator,
    ...
    TINYSWIFT_CHECK(exports_id == InstBlockId::Exports);
    auto imports_id = AddPlaceholder();
    TINYSWIFT_CHECK(imports_id == InstBlockId::Imports);
    auto global_init_id = AddPlaceholder();
    TINYSWIFT_CHECK(global_init_id == InstBlockId::GlobalInit);
  }
  ...
  auto ReplacePlaceholder(InstBlockId block_id, llvm::ArrayRef<InstId> content)
      -> void {
    TINYSWIFT_CHECK(block_id != InstBlockId::Empty);
    TINYSWIFT_CHECK(Get(block_id).empty(),
...
inline auto TinySwiftHashValue(const Inst& value, uint64_t seed) -> HashCode {
  Hasher hasher(seed);
...
#endif  // TINYSWIFT_TOOLCHAIN_SEM_IR_INST_H_
