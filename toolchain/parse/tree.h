...
#ifndef TINYSWIFT_TOOLCHAIN_PARSE_TREE_H_
#define TINYSWIFT_TOOLCHAIN_PARSE_TREE_H_
...
class Tree : public Printable<Tree> {
 public:
  ...
  auto IsValid(IdT id) const -> bool {
    ...
    TINYSWIFT_DCHECK(node_kind(id) == T::Kind);
  ...
  auto As(NodeId n) const -> T {
    TINYSWIFT_DCHECK(n.has_value());
    TINYSWIFT_DCHECK(ConvertTo<T>::AllowedFor(node_kind(n)),
  ...
  class NodeImpl : public Printable<NodeImpl> {
   public:
    explicit NodeImpl(NodeKind kind, bool has_error, Lex::TokenIndex token)
      ...
      TINYSWIFT_DCHECK(token.index >= 0, "Unexpected token for node: {0}", token);
    }
...
#endif  // TINYSWIFT_TOOLCHAIN_PARSE_TREE_H_
