...
#ifndef TINYSWIFT_TOOLCHAIN_PARSE_NODE_KIND_H_
#define TINYSWIFT_TOOLCHAIN_PARSE_NODE_KIND_H_
...
TINYSWIFT_DEFINE_RAW_ENUM_CLASS(NodeKind, uint16_t) {
#define TINYSWIFT_PARSE_NODE_KIND(Name) TINYSWIFT_RAW_ENUM_ENUMERATOR(Name)
...
// returns `true`.
class NodeKind : public TINYSWIFT_ENUM_BASE(NodeKind) {
 public:
#define TINYSWIFT_PARSE_NODE_KIND(Name) TINYSWIFT_ENUM_CONSTANT_DECL(Name)
#include "toolchain/parse/node_kind.def"
...
#define TINYSWIFT_PARSE_NODE_KIND(Name) \
  TINYSWIFT_ENUM_CONSTANT_DEFINITION(NodeKind, Name)
#include "toolchain/parse/node_kind.def"

inline constexpr int NodeKind::ValidCount = 0
#define TINYSWIFT_PARSE_NODE_KIND(Name) +1
#include "toolchain/parse/node_kind.def"
    ;
...
class NodeKind::Definition : public NodeKind {
 public:
  ...
  constexpr auto bracket() const -> NodeKind {
    TINYSWIFT_CHECK(has_bracket(), "{0}", *this);
    return bracket_;
  }
  ...
  constexpr auto child_count() const -> int32_t {
    TINYSWIFT_CHECK(has_child_count(), "{0}", *this);
    return child_count_;
  }
 ...
 private:
  ...
  auto MustSpecifyEitherBracketingNodeOrChildCount() -> void {
    TINYSWIFT_FATAL("Must specify either bracketing node or fixed child count.");
  }
...
#endif  // TINYSWIFT_TOOLCHAIN_PARSE_NODE_KIND_H_
