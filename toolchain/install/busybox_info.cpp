...
static auto GetMode(const std::filesystem::path& argv0)
  ...
  if (filename != "tinyswift" && filename != "tinyswift-busybox") {
    return filename;
  }
  return std::nullopt;
}
...
auto GetBusyboxInfo(const char* argv0) -> ErrorOr<BusyboxInfo> {
  ...
  while (true) {
    if (info.bin_path.filename() == "tinyswift-busybox") {
      // Check for bazel structure. For example, this makes work:
      //   /bin/sh -c "exec -a tinyswift ./bazel-bin/toolchain/tinyswift"
      //   /bin/sh -c "exec -a llvm-symbolizer ./bazel-bin/toolchain/tinyswift"
      //
      ...
      std::string busybox_path = info.bin_path.parent_path().string() +
                                 "/prefix/lib/tinyswift/tinyswift-busybox";
    ...
    // - TinySwift's CLI will be: `<prefix>/bin/tinyswift`
    // - Other tools will be: `<prefix>/lib/tinyswift/<group>/bin/<tool>`
    //
    ...
    if (parent_path.filename() == "bin") {
      auto lib_path = info.bin_path.filename() == "tinyswift"
                          ? parent_path / ".." / "lib" / "tinyswift"
                          : parent_path / ".." / "..";
      auto busybox_path = lib_path / "tinyswift-busybox";
      if (auto access = Filesystem::Cwd().Access(busybox_path);
          access.ok() && *access) {
        info.bin_path = busybox_path;
        return info;
    ...
    if (!readlink.ok()) {
      return ErrorBuilder()
             << "expected tinyswift-busybox symlink at `" << info.bin_path << "`";
    }
