...
//   TinySwift installation that should be added to the digest.
// - A required positional argument of an output file for the digest.
//
// The program reads the manifest of all the files in the TinySwift installation,
...
auto DigestProgram::Run(int argc, char** argv) -> ErrorOr<int> {
  ...
  TINYSWIFT_ASSIGN_OR_RETURN(std::string manifest,
  ...
  for (llvm::StringRef manifest_line : manifest_lines) {
    ...
    TINYSWIFT_VLOG("Digesting file: {0}\n", install_path);
  ...
  TINYSWIFT_VLOG("Digest: {0}\n", llvm::toHex(digest));

  TINYSWIFT_RETURN_IF_ERROR(Filesystem::Cwd().WriteFileFromString(
...
auto main(int argc, char** argv) -> int {
  TinySwift::DigestProgram program;
