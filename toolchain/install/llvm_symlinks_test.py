...
class LLVMSymlinksTest(unittest.TestCase):
    def setUp(self) -> None:
        ...
        self.prebuilt_runtimes = self.runfiles.Rlocation(
            "tinyswift/toolchain/driver/prebuilt_runtimes_tree"
        )
    ...
    def get_link_cmd(self, clang: Path) -> list[str | Path]:
        return [
            ...
            "-v",
            # Pass a parameter to the underlying TinySwift busybox using `-Xtinyswift`
            # to switch it to use the prebuilt runtimes rather than building
            # runtimes on demand.
            f"-Xtinyswift=--prebuilt-runtimes={self.prebuilt_runtimes}",
    ...
    # developing TinySwift, and encode all the different platform results in the
    # test expectations.
    def test_clang(self) -> None:
        bin = self.install_root / "lib/tinyswift/llvm/bin/clang"
    ...
    def test_clang_cl(self) -> None:
        bin = self.install_root / "lib/tinyswift/llvm/bin/clang-cl"
        run = subprocess.run(
    ...
    def test_clang_cpp(self) -> None:
        ...
        # check is expanded.
        bin = self.install_root / "lib/tinyswift/llvm/bin/clang-cpp"
        try:
            run = subprocess.run(
