...
"""Starlark cc_toolchain configuration rules for using the TinySwift toolchain"""

load(
    "@tinyswift_toolchain_config//:tinyswift_detected_variables.bzl",
    "clang_include_dirs",
    "clang_sysroot",
)
load(
...
def _impl(ctx):
    ...
    # Only use a sysroot if a non-trivial one is set in TinySwift's config.
    builtin_sysroot = None
    if clang_sysroot != "None" and clang_sysroot != "/":
        builtin_sysroot = clang_sysroot

    identifier = "tinyswift-toolchain-{0}-{1}".format(ctx.attr.target_cpu, ctx.attr.target_os)
...
tinyswift_cc_toolchain_config = rule(
...
def tinyswift_cc_toolchain_suite(name, configs):
