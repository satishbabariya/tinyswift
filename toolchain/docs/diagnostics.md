...
-   [CARBON_DIAGNOSTIC placement](#tinyswift_diagnostic-placement)
...
Note that `SortingConsumer` is used by default by `tinyswift compile`. In cases
...
`tinyswift compile –stream-errors`.
...
-   For formatting types, use the following, in order of preference:
    ...
    When a type is formatted within a larger snippet of TinySwift code, it can be
...
We want TinySwift's diagnostics to be helpful for developers when they run into an
error, and phrased consistently across diagnostics. In addition, TinySwift
...
TinySwift's diagnostic style aims to balance these concerns. Our style is:
