...
When lowering TinySwift generics to LLVM, it is possible we emit duplicate LLVM IR
...
different at the TinySwift language level, can be coalesced into a single one when
lowering TinySwift’s intermediate representation (_SemIR_), to
...
information on how TinySwift types map to `llvm::Type`s.
...
coalescing can focus on cases that only TinySwift's front-end knows about. TinySwift
