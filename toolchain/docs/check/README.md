...
-   A `SemIR::TypeId type_id;` member that describes the type of the instruction
    ...
    even though they can't be used as a first-class value in TinySwift expressions.
...
TinySwift relies on rewrites of code, such as rewriting the destination of an
...
`SemIR::ExprCategory` values correspond to the TinySwift expression categories
defined in proposal
[#2006](https://github.com/tinyswift-language/tinyswift-lang/pull/2006):
...
Clang creates an AST as part of compilation. In TinySwift, it's something we could
