...
As of [#3534](https://github.com/tinyswift-language/tinyswift-lang/pull/3534):
...
[#3310](https://github.com/tinyswift-language/tinyswift-lang/pull/3310):
...
There are several supported ways to run TinySwift on a given test file. For
example, with `toolchain/parse/testdata/basics/empty.tinyswift`:

-   `bazel test //toolchain/testing:file_test --test_arg=--file_tests=toolchain/parse/testdata/basics/empty.tinyswift`
    -   Executes an individual test.
-   `bazel run //toolchain -- compile --phase=parse --dump-parse-tree toolchain/parse/testdata/basics/empty.tinyswift`
    -   Explicitly runs `tinyswift` with the provided arguments.
-   `bazel-bin/toolchain/tinyswift compile --phase=parse --dump-parse-tree toolchain/parse/testdata/basics/empty.tinyswift`
    -   Similar to the previous command, but without using `bazel run`. This can
        ...
        binary.
-   `bazel run //toolchain -- -v compile --phase=check toolchain/check/testdata/basics/run.tinyswift`
...
`check/testdata/facet_types/min_prelude/my_test.tinyswift` might contain:

```
// INCLUDE-FILE: toolchain/testing/testdata/min_prelude/convert.tinyswift
```
