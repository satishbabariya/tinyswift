...
`bazel-bin/toolchain/tinyswift`. For the best debugging experience you may want to
...
```shell
lldb bazel-bin/toolchain/tinyswift
```
...
`~/.cache/tinyswift-lang-build-cache`. Deleting the disk cache, followed by a
...
2. Open a `.tinyswift` testdata file to debug. This must be the active file in VS
   Code.
...
subcommand name: `tinyswift -v compile ...`. `CARBON_VLOG` is used to print output
