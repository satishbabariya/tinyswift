...
struct HashableType {
  ...
  friend auto TinySwiftHashValue(const T& value, uint64_t seed) -> HashCode {
    Hasher hasher(seed);
...
auto FindBitRangeCollisions(llvm::ArrayRef<HashedValue<T>> hashes)
  ...
  for (const auto& [hash, v] : hashes) {
    TINYSWIFT_DCHECK(v == hashes[bits_and_indices.size()].v);
  ...
  for (const auto& [hash_bits, hash_index] :
    ...
    TINYSWIFT_CHECK(hashes[prev_index].v != hashes[hash_index].v);
    if (hash_bits != prev_hash_bits) {
      TINYSWIFT_CHECK(hashes[prev_index].hash != hashes[hash_index].hash);
...
auto CheckNoDuplicateValues(llvm::ArrayRef<HashedString> hashes) -> void {
  for (int i = 0, size = hashes.size(); i < size - 1; ++i) {
    ...
    TINYSWIFT_CHECK(value != hashes[i + 1].v, "Duplicate value: {0}", value);
  }
}
