...
struct IsTinySwiftSetImpl : std::false_type {};
template <typename KT, int MinSmallSize>
struct IsTinySwiftSetImpl<Set<KT, MinSmallSize>> : std::true_type {};

template <typename SetT>
static constexpr bool IsTinySwiftSet = IsTinySwiftSetImpl<SetT>::value;
...
// Provide a way to override the TinySwift Set specific benchmark runs with another
// hashtable implementation. When building, you can use one of these enum names
// in a macro define such as `-DTINYSWIFT_SET_BENCH_OVERRIDE=Name` in order to
// trigger a specific override for the `Set` type benchmarks. This is used to
// get before/after runs that compare the performance of TinySwift's Set versus
...
struct SetWrapperOverride<Set<KeyT, MinSmallSize>,
                          SetOverride::LLVMAndTinySwiftHash>
    : SetWrapperImpl<llvm::DenseSet<KeyT, TinySwiftHashDI<KeyT>>> {};

#ifndef TINYSWIFT_SET_BENCH_OVERRIDE
template <typename SetT>
using SetWrapper = SetWrapperImpl<SetT>;
#else
template <typename SetT>
using SetWrapper =
    SetWrapperOverride<SetT, SetOverride::TINYSWIFT_SET_BENCH_OVERRIDE>;
#endif
...
#define MAP_BENCHMARK_ONE_OP_SIZE(NAME, APPLY, KT)        \
  ...
  BENCHMARK(NAME<llvm::DenseSet<KT, TinySwiftHashDI<KT>>>)->Apply(APPLY)
...
#define MAP_BENCHMARK_OP_SEQ_SIZE(NAME, KT)                  \
  ...
  BENCHMARK(NAME<llvm::DenseSet<KT, TinySwiftHashDI<KT>>>)->Apply(SizeArgs)
// NOLINTEND(bugprone-macro-parentheses)
...
static void BM_SetInsertSeq(benchmark::State& state) {
  ...
  if constexpr (IsTinySwiftSet<SetT>) {
