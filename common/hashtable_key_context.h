...
#ifndef TINYSWIFT_COMMON_HASHTABLE_KEY_CONTEXT_H_
#define TINYSWIFT_COMMON_HASHTABLE_KEY_CONTEXT_H_
...
// found by ADL for your type called `TinySwiftHashtableEq` with the following
// signature:
//
// ```cpp
// auto TinySwiftHashtableEq(const YourType& lhs, const YourType& rhs) -> bool;
// ```
//
...
//
// The default for hashing uses TinySwift's `hashing.h`. Customizations must still
...
inline auto TinySwiftHashtableEq(const llvm::APInt& lhs, const llvm::APInt& rhs)
    -> bool {
  return lhs.getBitWidth() == rhs.getBitWidth() && lhs == rhs;
}

inline auto TinySwiftHashtableEq(const llvm::APFloat& lhs,
                              const llvm::APFloat& rhs) -> bool {
  return lhs.bitwiseIsEqual(rhs);
}

template <typename LeftT, typename RightT>
inline auto TinySwiftHashtableEq(const LeftT& lhs, const RightT& rhs) -> bool
  requires(requires {
...
inline auto DispatchImpl(const LeftT& lhs, const RightT& rhs) -> bool {
  ...
  return TinySwiftHashtableEq(lhs, rhs);
}
...
#endif  // TINYSWIFT_COMMON_HASHTABLE_KEY_CONTEXT_H_
