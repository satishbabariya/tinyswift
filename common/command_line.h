...
#ifndef TINYSWIFT_COMMON_COMMAND_LINE_H_
#define TINYSWIFT_COMMON_COMMAND_LINE_H_
...
auto ArgBuilder::MetaAction(T action) -> void {
  TINYSWIFT_CHECK(!arg_->meta_action, "Cannot set a meta action twice!");
...
auto OneOfArgBuilder::OneOfImpl(const OneOfValueT<U> (&input_values)[N],
  ...
  if ((input_values[Indices].is_default || ...)) {
    TINYSWIFT_CHECK(!arg()->is_append, "Can't append default.");
    TINYSWIFT_CHECK((input_values[Indices].is_default + ... + 0) == 1,
...
#endif  // TINYSWIFT_COMMON_COMMAND_LINE_H_
