...
#ifndef TINYSWIFT_COMMON_RAW_HASHTABLE_BENCHMARK_HELPERS_H_
#define TINYSWIFT_COMMON_RAW_HASHTABLE_BENCHMARK_HELPERS_H_
...
struct LowZeroBitInt {
  ...
  friend auto TinySwiftHashValue(const LowZeroBitInt& value, uint64_t seed)
      -> HashCode {
...
// using TinySwift's hashing framework. These let us benchmark the data structure
...
// for broader use. The TinySwift hashing infrastructure has only been evaluated in
// the context of its specific hashtable design.
template <typename T>
struct TinySwiftHashDI;

template <>
struct TinySwiftHashDI<int> {
...
template <int LowZeroBits>
struct TinySwiftHashDI<LowZeroBitInt<LowZeroBits>> {
...
template <>
struct TinySwiftHashDI<llvm::StringRef> {
...
template <int LowZeroBits>
struct DenseMapInfo<TinySwift::RawHashtable::LowZeroBitInt<LowZeroBits>> {
  using IntT = TinySwift::RawHashtable::LowZeroBitInt<LowZeroBits>;
  static auto getEmptyKey() -> IntT { return IntT(-1); }
  static auto getTombstoneKey() -> IntT { return IntT(-2); }
  static auto getHashValue(const IntT val) -> unsigned {
...
#endif  // TINYSWIFT_COMMON_RAW_HASHTABLE_BENCHMARK_HELPERS_H_
