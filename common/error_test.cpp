...
using ::TinySwift::Testing::IsError;
using ::TinySwift::Testing::IsSuccess;
using ::testing::_;
using ::testing::Eq;
...
TYPED_TEST(ErrorOrTest, ReturnIfErrorNoError) {
  ...
  auto result = []() -> TestErrorOr {
    TINYSWIFT_RETURN_IF_ERROR(TestErrorOr(Success()));
    TINYSWIFT_RETURN_IF_ERROR(TestErrorOr(Success()));
    return Success();
  }();
...
TYPED_TEST(ErrorOrTest, ReturnIfErrorHasError) {
  ...
  auto result = [this]() -> TestErrorOr {
    TINYSWIFT_RETURN_IF_ERROR(TestErrorOr(Success()));
    TINYSWIFT_RETURN_IF_ERROR(TestErrorOr(this->MakeError()));
    return Success();
  }();
...
TYPED_TEST(ErrorOrTest, AssignOrReturnNoError) {
  using TestErrorOr = ErrorOr<int, TypeParam>;
  auto result = []() -> TestErrorOr {
    TINYSWIFT_ASSIGN_OR_RETURN(int a, TestErrorOr(1));
    TINYSWIFT_ASSIGN_OR_RETURN(const int b, TestErrorOr(2));
    int c = 0;
    TINYSWIFT_ASSIGN_OR_RETURN(c, TestErrorOr(3));
    return a + b + c;
  }();
...
TYPED_TEST(ErrorOrTest, AssignOrReturnHasErrorInExpected) {
  using TestErrorOr = ErrorOr<int, TypeParam>;
  auto result = [this]() -> TestErrorOr {
    TINYSWIFT_ASSIGN_OR_RETURN(int a, TestErrorOr(this->MakeError()));
    return a;
  }();
  EXPECT_THAT(result, IsError(this->ErrorStr()));
}
...
TYPED_TEST(ErrorOrTest, AssignOrReturnNoErrorAcrossErrorTypes) {
  ...
  auto result = []() -> ErrorOr<int> {
    TINYSWIFT_ASSIGN_OR_RETURN(int a, TestErrorOr(1));
    TINYSWIFT_ASSIGN_OR_RETURN(const int b, []() -> TestErrorOr {
      TINYSWIFT_ASSIGN_OR_RETURN(int inner, (ErrorOr<int, AnotherCustomError>(2)));
      return inner;
    }());
    int c = 0;
    TINYSWIFT_ASSIGN_OR_RETURN(c, TestErrorOr(3));
