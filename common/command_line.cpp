...
namespace TinySwift::CommandLine {
...
auto operator<<(llvm::raw_ostream& output, ParseResult result)
  ...
  TINYSWIFT_FATAL("Corrupt parse result!");
}
...
auto operator<<(llvm::raw_ostream& output, ArgKind kind) -> llvm::raw_ostream& {
  ...
  TINYSWIFT_FATAL("Corrupt argument kind!");
}
...
auto operator<<(llvm::raw_ostream& output, CommandKind kind)
  ...
  TINYSWIFT_FATAL("Corrupt command kind!");
}
...
auto MetaPrinter::PrintOptionShortName(const Arg& arg) const -> void {
  TINYSWIFT_CHECK(!arg.info.short_name.empty(), "No short name to use.");
...
auto MetaPrinter::PrintArgHelp(const Arg& arg, llvm::StringRef indent) const
  ...
  switch (arg.kind) {
    ...
    case Arg::Kind::Invalid:
      TINYSWIFT_FATAL("Argument configured without any action or kind!");
  }
}
...
auto Parser::SetOptionDefault(const Arg& option) -> void {
  TINYSWIFT_CHECK(option.has_default, "No default value available!");
  switch (option.kind) {
    case Arg::Kind::Flag:
    ...
    case Arg::Kind::OneOf:
      ...
      break;
    case Arg::Kind::MetaActionOnly:
      TINYSWIFT_FATAL("Can't set a default value for a meta action!");
    case Arg::Kind::Invalid:
      TINYSWIFT_FATAL("Option configured without any action or kind!");
  }
}
...
auto Parser::ParseFlag(const Arg& flag, std::optional<llvm::StringRef> value)
    -> ErrorOr<Success> {
  TINYSWIFT_CHECK(flag.kind == Arg::Kind::Flag, "Incorrect kind: {0}", flag.kind);
  if (!value || *value == "true") {
...
auto Parser::ParseIntegerArgValue(const Arg& arg, llvm::StringRef value)
    -> ErrorOr<Success> {
  TINYSWIFT_CHECK(arg.kind == Arg::Kind::Integer, "Incorrect kind: {0}", arg.kind);
  int integer_value;
...
auto Parser::ParseStringArgValue(const Arg& arg, llvm::StringRef value)
    -> ErrorOr<Success> {
  TINYSWIFT_CHECK(arg.kind == Arg::Kind::String, "Incorrect kind: {0}", arg.kind);
  if (!arg.is_append) {
...
auto Parser::ParseOneOfArgValue(const Arg& arg, llvm::StringRef value)
    -> ErrorOr<Success> {
  TINYSWIFT_CHECK(arg.kind == Arg::Kind::OneOf, "Incorrect kind: {0}", arg.kind);
...
auto Parser::ParseLongOption(llvm::StringRef unparsed_arg) -> ErrorOr<Success> {
  TINYSWIFT_CHECK(unparsed_arg.starts_with("--") && unparsed_arg.size() > 2,
...
auto Parser::ParseShortOptionSeq(llvm::StringRef unparsed_arg)
    -> ErrorOr<Success> {
  TINYSWIFT_CHECK(unparsed_arg.starts_with("-") && unparsed_arg.size() > 1,
  ...
  for (unsigned char c : unparsed_arg) {
    ...
    TINYSWIFT_RETURN_IF_ERROR(ParseArg(arg, /*short_spelling=*/true, value));
  }
  return Success();
}
...
auto Parser::ParseSubcommand(llvm::StringRef unparsed_arg) -> ErrorOr<Success> {
  ...
  TINYSWIFT_RETURN_IF_ERROR(FinalizeParsedOptions());
...
auto Parser::FinalizeParse() -> ErrorOr<ParseResult> {
  ...
  switch (command_->kind) {
    case Command::Kind::Invalid:
      TINYSWIFT_FATAL("Should never have a parser with an invalid command!");
...
auto Parser::ParsePositionalSuffix(
  ...
  while (!unparsed_args.empty()) {
    ...
    if (unparsed_arg != "--") {
      TINYSWIFT_RETURN_IF_ERROR(ParsePositionalArg(unparsed_arg));
...
auto Parser::Parse(llvm::ArrayRef<llvm::StringRef> unparsed_args)
  ...
  while (!unparsed_args.empty()) {
    ...
    if (unparsed_args.front() == "--") {
      ...
      TINYSWIFT_RETURN_IF_ERROR(ParsePositionalSuffix(unparsed_args));
    ...
    if (unparsed_arg.starts_with("--")) {
      ...
      TINYSWIFT_RETURN_IF_ERROR(ParseLongOption(unparsed_arg));
      continue;
    }

    if (unparsed_arg.starts_with("-") && unparsed_arg.size() > 1) {
      TINYSWIFT_RETURN_IF_ERROR(ParseShortOptionSeq(unparsed_arg));
      continue;
    }
    ...
    TINYSWIFT_RETURN_IF_ERROR(ParseSubcommand(unparsed_arg));
  }
...
auto CommandBuilder::AddSubcommand(
  ...
  TINYSWIFT_CHECK(IsValidName(info.name), "Invalid subcommand name: {0}",
               info.name);
  TINYSWIFT_CHECK(subcommand_names_.insert(info.name).second,
...
auto CommandBuilder::RequiresSubcommand() -> void {
  TINYSWIFT_CHECK(!command_->subcommands.empty(),
               "Cannot require subcommands unless there are subcommands.");
  TINYSWIFT_CHECK(command_->positional_args.empty(),
               "Cannot require subcommands and have a positional argument.");
  TINYSWIFT_CHECK(command_->kind == Kind::Invalid,
...
auto CommandBuilder::AddArgImpl(const ArgInfo& info, Arg::Kind kind) -> Arg* {
  TINYSWIFT_CHECK(IsValidName(info.name), "Invalid argument name: {0}", info.name);
  TINYSWIFT_CHECK(arg_names_.insert(info.name).second,
...
auto CommandBuilder::AddPositionalArgImpl(
  ...
  TINYSWIFT_CHECK(!arg.is_help_hidden,
               "Cannot have a help-hidden positional argument.");

  if (arg.is_required && command_->positional_args.size() > 1) {
    TINYSWIFT_CHECK((*std::prev(command_->positional_args.end(), 2))->is_required,
