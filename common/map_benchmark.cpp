...
using RawHashtable::TinySwiftHashDI;
...
struct IsTinySwiftMapImpl : std::false_type {};
template <typename KT, typename VT, int MinSmallSize>
struct IsTinySwiftMapImpl<Map<KT, VT, MinSmallSize>> : std::true_type {};

template <typename MapWrapperT>
static constexpr bool IsTinySwiftMap =
    IsTinySwiftMapImpl<typename MapWrapperT::MapT>::value;
...
// Explicit (partial) specialization for the TinySwift map type that uses its
...
// Provide a way to override the TinySwift Map specific benchmark runs with another
// hashtable implementation. When building, you can use one of these enum names
// in a macro define such as `-DTINYSWIFT_MAP_BENCH_OVERRIDE=Name` in order to
// trigger a specific override for the `Map` type benchmarks. This is used to
// get before/after runs that compare the performance of TinySwift's Map versus
...
enum class MapOverride : uint8_t {
  ...
  Boost,
  LLVM,
  LLVMAndTinySwiftHash,
};
#ifndef TINYSWIFT_MAP_BENCH_OVERRIDE
#define TINYSWIFT_MAP_BENCH_OVERRIDE None
#endif
...
struct MapWrapperOverride<Map<KeyT, ValueT, MinSmallSize>,
                          MapOverride::LLVMAndTinySwiftHash>
    : MapWrapperImpl<llvm::DenseMap<KeyT, ValueT, TinySwiftHashDI<KeyT>>> {};

template <typename MapT>
using MapWrapper =
    MapWrapperOverride<MapT, MapOverride::TINYSWIFT_MAP_BENCH_OVERRIDE>;

template <typename MapT>
auto ReportMetrics(const MapWrapper<MapT>& m_wrapper, benchmark::State& state)
    -> void {
  // Report some extra statistics about the TinySwift type.
  if constexpr (IsTinySwiftMap<MapWrapper<MapT>>) {
...
#define MAP_BENCHMARK_ONE_OP_SIZE(NAME, APPLY, KT, VT)                         \
  ...
  BENCHMARK(NAME<llvm::DenseMap<KT, VT, TinySwiftHashDI<KT>>>)->Apply(APPLY)
...
// challenges to the hashing strategy TinySwift's hash tables use and so they help
...
#define MAP_BENCHMARK_OP_SEQ_SIZE(NAME, KT, VT)                  \
  ...
  BENCHMARK(NAME<boost::unordered::unordered_flat_map<KT, VT>>)  \
      ->Apply(SizeArgs);                                         \
  BENCHMARK(NAME<llvm::DenseMap<KT, VT>>)->Apply(APPLY);         \
  BENCHMARK(NAME<llvm::DenseMap<KT, VT, TinySwiftHashDI<KT>>>)->Apply(SizeArgs)
// NOLINTEND(bugprone-macro-parentheses)
...
// probed keys for TinySwift's set as that is both a general reflection of the
...
static void BM_MapInsertSeq(benchmark::State& state) {
  ...
  for (auto _ : state) {
    ...
    for (auto k : keys) {
      ...
      TINYSWIFT_DCHECK(inserted, "Must be a successful insert!");
    }

    // Now insert a final random repeated key.
    bool inserted = m.BenchInsert(lookup_keys[i], MakeValue2<VT>());
    TINYSWIFT_DCHECK(!inserted, "Must already be in the map!");
  ...
  // Report some extra statistics about the TinySwift type.
  if constexpr (IsTinySwiftMap<MapWrapperT>) {
