...
#define TINYSWIFT_TEST_KIND(X) \
  X(Beep)                   \
  X(Boop)                   \
  X(Burr)

TINYSWIFT_DEFINE_RAW_ENUM_MASK(TestKind, uint8_t) {
  TINYSWIFT_TEST_KIND(TINYSWIFT_RAW_ENUM_MASK_ENUMERATOR)
};

class TestKind : public TINYSWIFT_ENUM_MASK_BASE(TestKind) {
 public:
  TINYSWIFT_TEST_KIND(TINYSWIFT_ENUM_MASK_CONSTANT_DECL)

  using EnumMaskBase::AsInt;
  using EnumMaskBase::FromInt;
};

#define TINYSWIFT_TEST_KIND_WITH_TYPE(X) \
  TINYSWIFT_ENUM_MASK_CONSTANT_DEFINITION(TestKind, X)
TINYSWIFT_TEST_KIND(TINYSWIFT_TEST_KIND_WITH_TYPE)
#undef TINYSWIFT_TEST_KIND_WITH_TYPE

TINYSWIFT_DEFINE_ENUM_MASK_NAMES(TestKind) {
  TINYSWIFT_TEST_KIND(TINYSWIFT_ENUM_MASK_NAME_STRING)
};
