...
#ifndef TINYSWIFT_COMMON_RAW_HASHTABLE_H_
#define TINYSWIFT_COMMON_RAW_HASHTABLE_H_
...
#ifndef TINYSWIFT_ENABLE_PREFETCH_METADATA
#define TINYSWIFT_ENABLE_PREFETCH_METADATA 0
#endif
#ifndef TINYSWIFT_ENABLE_PREFETCH_ENTRY_GROUP
#define TINYSWIFT_ENABLE_PREFETCH_ENTRY_GROUP 1
#endif
...
class ViewImpl {
 ...
 private:
  ...
  static constexpr auto EntriesOffset(ssize_t alloc_size) -> ssize_t {
    TINYSWIFT_DCHECK(llvm::isPowerOf2_64(alloc_size),
    ...
    // early.
    TINYSWIFT_DCHECK(static_cast<uint64_t>(alloc_size) ==
  ...
  auto PrefetchMetadata() const -> void {
    if constexpr (TINYSWIFT_ENABLE_PREFETCH_METADATA) {
  ...
  static auto PrefetchEntryGroup(const EntryT* entry_group) -> void {
    if constexpr (TINYSWIFT_ENABLE_PREFETCH_ENTRY_GROUP) {
      // Prefetch with a "low" temporal locality as we're primarily expecting a
...
class BaseImpl {
 protected:
  ...
  BaseImpl(int small_alloc_size, Storage* small_storage)
    ...
    TINYSWIFT_CHECK(small_alloc_size >= 0);
 ...
 private:
  ...
  auto is_small() const -> bool {
    TINYSWIFT_DCHECK(alloc_size() >= small_alloc_size());
...
inline auto ComputeProbeMaskFromSize(ssize_t size) -> size_t {
  TINYSWIFT_DCHECK(llvm::isPowerOf2_64(size),
                "Size must be a power of two for a hashed buffer!");
...
auto ViewImpl<InputKeyT, InputValueT, InputKeyContextT>::ComputeMetricsImpl(
  ...
  for (ssize_t group_index = 0; group_index < local_size;
    ...
    for (ssize_t byte_index : present_matched_range) {
      ...
      auto probe_g = MetadataGroup::Load(local_metadata, s.index());
      auto probe_matched_range = probe_g.Match(tag);
      TINYSWIFT_CHECK(!probe_matched_range.empty());
...
auto BaseImpl<InputKeyT, InputValueT, InputKeyContextT>::InsertImpl(
  ...
  for (ProbeSequence s(hash_index, alloc_size());; s.Next()) {
    ...
    --growth_budget_;
    TINYSWIFT_DCHECK(growth_budget() >= 0,
...
BaseImpl<InputKeyT, InputValueT, InputKeyContextT>::GrowToAllocSizeImpl(
  ...
  TINYSWIFT_CHECK(llvm::isPowerOf2_64(target_alloc_size));
...
auto BaseImpl<InputKeyT, InputValueT, InputKeyContextT>::GrowForInsertCountImpl(
  ...
  TINYSWIFT_CHECK(GrowthThresholdForAllocSize(target_alloc_size) >
               (budget_needed));
...
auto BaseImpl<InputKeyT, InputValueT, InputKeyContextT>::CopySlotsFrom(
  ...
  requires(EntryT::IsCopyable)
{
  TINYSWIFT_DCHECK(alloc_size() == arg.alloc_size());
...
auto BaseImpl<InputKeyT, InputValueT, InputKeyContextT>::MoveFrom(
  ...
  ssize_t local_size = alloc_size();
  TINYSWIFT_DCHECK(local_size == arg.alloc_size());
  ...
  if (arg.is_small()) {
    TINYSWIFT_DCHECK(local_size == small_alloc_size_);
...
auto BaseImpl<InputKeyT, InputValueT, InputKeyContextT>::ComputeNextAllocSize(
  ...
  TINYSWIFT_DCHECK(llvm::isPowerOf2_64(old_alloc_size),
                "Expected a power of two!");
  ssize_t new_alloc_size;
  bool overflow = __builtin_mul_overflow(old_alloc_size, 2, &new_alloc_size);
  TINYSWIFT_CHECK(!overflow, "Computing the new size overflowed `ssize_t`!");
...
auto BaseImpl<InputKeyT, InputValueT, InputKeyContextT>::GrowToNextAllocSize(
  ...
  for (ssize_t group_index = 0; group_index < old_size;
    ...
    for (ssize_t byte_index : present_matched_range) {
      ...
      if constexpr (!MetadataGroup::FastByteClear) {
        TINYSWIFT_DCHECK(new_metadata[old_index] == old_metadata[old_index]);
        TINYSWIFT_DCHECK(new_metadata[old_index | old_size] ==
      ...
      TINYSWIFT_DCHECK(new_index == old_hash_index ||
  ...
  TINYSWIFT_DCHECK((count - static_cast<ssize_t>(probed_indices.size())) ==
                (new_size - llvm::count(llvm::ArrayRef(new_metadata, new_size),
                                        MetadataGroup::Empty)));
#ifndef NDEBUG
  TINYSWIFT_DCHECK((debug_empty_count + debug_deleted_count) ==
                (old_size - count));
  TINYSWIFT_DCHECK(llvm::count(llvm::ArrayRef(new_metadata, new_size),
                            MetadataGroup::Empty) ==
  ...
  TINYSWIFT_DCHECK(count ==
                (new_size - llvm::count(llvm::ArrayRef(new_metadata, new_size),
                                        MetadataGroup::Empty)));
  growth_budget_ -= count;
  TINYSWIFT_DCHECK(growth_budget_ ==
                (GrowthThresholdForAllocSize(new_size) -
                 (new_size - llvm::count(llvm::ArrayRef(new_metadata, new_size),
                                         MetadataGroup::Empty))));
  TINYSWIFT_DCHECK(growth_budget_ > 0 &&
...
TableImpl<InputBaseT, SmallSize>::TableImpl(const TableImpl& arg)
  ...
  TINYSWIFT_DCHECK(arg.alloc_size() == 0 || !arg.is_small() ||
                arg.alloc_size() == SmallSize);
  TINYSWIFT_DCHECK(arg.small_alloc_size_ == SmallSize);
  TINYSWIFT_DCHECK(this->small_alloc_size_ == SmallSize);
  ...
  requires(BaseT::EntryT::IsCopyable)
{
  ...
  if (arg.alloc_size() == this->alloc_size()) {
    ...
    TINYSWIFT_DCHECK(arg.storage() != this->storage());
    if constexpr (!EntryT::IsTriviallyDestructible) {
...
auto TableImpl<InputBaseT, SmallSize>::ResetImpl() -> void {
  ...
  TINYSWIFT_DCHECK(this->small_alloc_size() == SmallSize);
...
#endif  // TINYSWIFT_COMMON_RAW_HASHTABLE_H_
