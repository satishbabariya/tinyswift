...
#ifndef TINYSWIFT_COMMON_ERROR_H_
#define TINYSWIFT_COMMON_ERROR_H_
...
class [[nodiscard]] Error : public Printable<Error> {
 public:
  ...
  explicit Error(llvm::Twine message) : message_(message.str()) {
    TINYSWIFT_CHECK(!message_.empty(), "Errors must have a message.");
  }
...
class [[nodiscard]] ErrorOr {
 public:
  ...
  // REQUIRES: `ok()` is true.
  auto Check() const -> void { TINYSWIFT_CHECK(ok(), "{0}", error()); }
...
#define TINYSWIFT_MAKE_UNIQUE_NAME_IMPL(a, b, c) a##b##c
#define TINYSWIFT_MAKE_UNIQUE_NAME(a, b, c) TINYSWIFT_MAKE_UNIQUE_NAME_IMPL(a, b, c)

// Macro to prevent a top-level comma from being interpreted as a macro
// argument separator.
#define TINYSWIFT_PROTECT_COMMAS(...) __VA_ARGS__

#define TINYSWIFT_RETURN_IF_ERROR_IMPL(unique_name, expr)  \
  if (auto unique_name = (expr); !(unique_name).ok()) { \
    return std::move(unique_name).error();              \
  }

#define TINYSWIFT_RETURN_IF_ERROR(expr)                                    \
  TINYSWIFT_RETURN_IF_ERROR_IMPL(                                          \
      TINYSWIFT_MAKE_UNIQUE_NAME(_llvm_error_line, __LINE__, __COUNTER__), \
      TINYSWIFT_PROTECT_COMMAS(expr))

#define TINYSWIFT_ASSIGN_OR_RETURN_IMPL(unique_name, var, expr) \
...
#define TINYSWIFT_ASSIGN_OR_RETURN(var, expr)                                 \
  TINYSWIFT_ASSIGN_OR_RETURN_IMPL(                                            \
      TINYSWIFT_MAKE_UNIQUE_NAME(_llvm_expected_line, __LINE__, __COUNTER__), \
      TINYSWIFT_PROTECT_COMMAS(var), TINYSWIFT_PROTECT_COMMAS(expr))

#endif  // TINYSWIFT_COMMON_ERROR_H_
