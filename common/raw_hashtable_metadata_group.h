...
#ifndef TINYSWIFT_COMMON_RAW_HASHTABLE_METADATA_GROUP_H_
#define TINYSWIFT_COMMON_RAW_HASHTABLE_METADATA_GROUP_H_
...
#include <x86intrin.h>
#define TINYSWIFT_X86_SIMD_SUPPORT 1
#elif defined(__ARM_NEON)
#include <arm_neon.h>
#define TINYSWIFT_NEON_SIMD_SUPPORT 1
#endif
...
class BitIndex
 ...
 public:
  ...
  auto empty() const -> bool {
    TINYSWIFT_DCHECK((bits_ & ZeroMask) == 0, "Unexpected non-zero bits!");
  ...
  auto index() -> ssize_t {
    TINYSWIFT_DCHECK(bits_ != 0, "Cannot get an index from zero bits!");
  ...
  class Iterator
   ...
   public:
    ...
    auto operator++() -> Iterator& {
      TINYSWIFT_DCHECK(bits_ != 0, "Must not increment past the end!");
      __builtin_assume(bits_ != 0);
...
//
// The metadata bytes used TinySwift's hashtable implementation are designed to
...
// optimize the particular usages in TinySwift's hashtable and should not be
...
//     TINYSWIFT_DCHECK(result == portable_result, "{0}", ...);
//   }
...
#if TINYSWIFT_NEON_SIMD_SUPPORT
    uint8x8_t metadata_vec = {};
    static_assert(sizeof(metadata_vec) == Size);
#elif TINYSWIFT_X86_SIMD_SUPPORT
...
#if !defined(NDEBUG) && (TINYSWIFT_NEON_SIMD_SUPPORT || TINYSWIFT_X86_SIMD_SUPPORT)
      true;
#else
      false;
#endif
...
inline auto MetadataGroup::Store(uint8_t* metadata, ssize_t index) const
  ...
  TINYSWIFT_DCHECK(0 == std::memcmp(metadata + index, &metadata_bytes, Size));
}
...
inline auto MetadataGroup::Match(uint8_t tag) const -> MatchRange {
  ...
  // a present byte.
  TINYSWIFT_DCHECK((tag & PresentMask) == 0, "{0:x}", tag);
  ...
  if constexpr (UseSimd || DebugSimd) {
    ...
    TINYSWIFT_DCHECK(simd_result == portable_result,
...
inline auto MetadataGroup::VerifyPortableRangeBits(
  ...
  for (ssize_t byte_index : llvm::seq<ssize_t>(0, Size)) {
    if constexpr (!ByteEncoding) {
      if (byte_match(metadata_bytes[byte_index])) {
        TINYSWIFT_CHECK(((range_bits >> byte_index) & 1) == 1,
                     "Bit not set at matching byte index: {0}", byte_index);
      } else {
        TINYSWIFT_CHECK(((range_bits >> byte_index) & 1) == 0,
                     "Bit set at non-matching byte index: {0}", byte_index);
      }
    } else {
      ...
      if (byte_match(metadata_bytes[byte_index])) {
        TINYSWIFT_CHECK(range_byte == 0x80,
...
inline auto MetadataGroup::PortableMatch(uint8_t tag) const -> MatchRange {
  ...
  TINYSWIFT_DCHECK(broadcast == (Lsbs * tag | Msbs),
  ...
  TINYSWIFT_DCHECK(VerifyPortableRangeBits(
...
inline auto MetadataGroup::PortableMatchDeleted() const -> MatchIndex {
  ...
  TINYSWIFT_DCHECK(VerifyIndexBits(match_bits,
...
#endif  // TINYSWIFT_COMMON_RAW_HASHTABLE_METADATA_GROUP_H_
