...
struct TinySwiftHashBench : HashBenchBase {
  template <typename T>
...
#define LATENCY_VALUE_BENCHMARKS(...)                                  \
  BENCHMARK(BM_LatencyHash<RandValues<__VA_ARGS__>, TinySwiftHashBench>); \
...
// coverage here.
LATENCY_STRING_SIZE_BENCHMARKS(TinySwiftHashBench);
