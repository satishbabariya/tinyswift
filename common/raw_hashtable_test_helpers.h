...
#ifndef TINYSWIFT_COMMON_RAW_HASHTABLE_TEST_HELPERS_H_
#define TINYSWIFT_COMMON_RAW_HASHTABLE_TEST_HELPERS_H_
...
struct TestData : Printable<TestData> {
  ...
  TestData(int v) : value(v) { TINYSWIFT_CHECK(value >= 0); }
  ~TestData() {
    TINYSWIFT_CHECK(value >= 0);
    value = -1;
  }
  ...
  friend auto TinySwiftHashValue(TestData data, uint64_t seed) -> HashCode {
    return TinySwift::HashValue(data.value, seed);
  }
};
...
struct MoveOnlyTestData : Printable<TestData> {
  ...
  // NOLINTNEXTLINE: google-explicit-constructor
  MoveOnlyTestData(int v) : value(v) { TINYSWIFT_CHECK(value >= 0); }
  ~MoveOnlyTestData() {
    TINYSWIFT_CHECK(value >= 0);
    value = -1;
  }
  ...
  friend auto TinySwiftHashValue(const MoveOnlyTestData& data, uint64_t seed)
      -> HashCode {
    return TinySwift::HashValue(data.value, seed);
  }
};
...
struct FixedHashKeyContext : DefaultKeyContext {
  ...
  auto HashKey(const KeyT& key, uint64_t seed) const -> HashCode {
    ...
    if (FixIndexBits) {
      ...
      TINYSWIFT_DCHECK(HashCode(raw_hash).ExtractIndexAndTag<TagBits>().first ==
                    (FixedVal & (~static_cast<uint64_t>(0) >> TagBits)));
    }
    if (FixTagBits) {
      raw_hash &= ~TagMask;
      raw_hash |= FixedVal & TagMask;
      TINYSWIFT_DCHECK(HashCode(raw_hash).ExtractIndexAndTag<TagBits>().second ==
                    (FixedVal & TagMask));
    }
...
#endif  // TINYSWIFT_COMMON_RAW_HASHTABLE_TEST_HELPERS_H_
