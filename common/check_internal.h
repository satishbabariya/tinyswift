...
#ifndef TINYSWIFT_COMMON_CHECK_INTERNAL_H_
#define TINYSWIFT_COMMON_CHECK_INTERNAL_H_
...
CheckCondition(bool condition)
    ...
    __attribute__((diagnose_if(!condition,
                               ...
                               "TINYSWIFT_FATAL if this is intended",
                               "error"))) {
  return condition;
}
...
#define TINYSWIFT_INTERNAL_CHECK_CONDITION(cond) \
  (TinySwift::Internal::CheckCondition(true && (cond)))
...
#define TINYSWIFT_INTERNAL_CHECK_IMPL(kind, file, line, condition_str) \
  (TinySwift::Internal::CheckFail<kind, file, line, condition_str, "">())
...
#define TINYSWIFT_INTERNAL_CHECK_IMPL_FORMAT(kind, file, line, condition_str,   \
                                          format_str, ...)                   \
  (TinySwift::Internal::CheckFail<kind, file, line, condition_str, format_str>( \
      __VA_ARGS__))
...
#define TINYSWIFT_INTERNAL_CHECK(condition, ...)      \
  TINYSWIFT_INTERNAL_CHECK_IMPL##__VA_OPT__(_FORMAT)( \
...
#define TINYSWIFT_INTERNAL_FATAL(...)                                  \
  (TINYSWIFT_INTERNAL_CHECK_IMPL##__VA_OPT__(_FORMAT)(                 \
       "FATAL", __FILE__, __LINE__, "" __VA_OPT__(, ) __VA_ARGS__), \
   TINYSWIFT_INTERNAL_FATAL_NORETURN_SUFFIX())
...
#define TINYSWIFT_INTERNAL_DEAD_DCHECK(condition, ...) \
  TINYSWIFT_INTERNAL_DEAD_DCHECK_IMPL##__VA_OPT__(_FORMAT)(__VA_ARGS__)

#define TINYSWIFT_INTERNAL_DEAD_DCHECK_IMPL() \
  TinySwift::Internal::CheckFail<"", "", 0, "", "">()

#define TINYSWIFT_INTERNAL_DEAD_DCHECK_IMPL_FORMAT(format_str, ...) \
  TinySwift::Internal::CheckFail<"", "", 0, "", "">(__VA_ARGS__)

// The CheckFail function itself is noreturn in NDEBUG.
#define TINYSWIFT_INTERNAL_FATAL_NORETURN_SUFFIX() void()
#else
#define TINYSWIFT_INTERNAL_FATAL_NORETURN_SUFFIX() std::abort()
#endif

#endif  // TINYSWIFT_COMMON_CHECK_INTERNAL_H_
