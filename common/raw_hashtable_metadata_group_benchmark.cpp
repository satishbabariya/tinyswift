...
#if TINYSWIFT_NEON_SIMD_SUPPORT || TINYSWIFT_X86_SIMD_SUPPORT
...
#pragma clang loop unroll(disable)
  for (auto _ : s) {
    ...
    // That exactly matches how the `TINYSWIFT_CHECK` macro works, and so this
    // serves as both a good correctness test and replication of hashtable usage
    // of a match.
    TINYSWIFT_CHECK(matches);
...
#pragma clang loop unroll(disable)
  for (auto _ : s) {
    ...
    // We don't use a `TINYSWIFT_CHECK` here as the loop below will test the range
...
#if TINYSWIFT_USE_X86_SIMD_CONTROL_GROUP
