...
#ifndef TINYSWIFT_COMMON_BAZEL_WORKING_DIR_H_
#define TINYSWIFT_COMMON_BAZEL_WORKING_DIR_H_
...
inline auto SetWorkingDirForBazelRun(std::filesystem::path exe_path = {})
  ...
  if (!exe_path.empty()) {
    ...
    TINYSWIFT_CHECK(!err, "Unable to make an absolute path for `{0}`: {1}",
                 exe_path, err.message());
  }

  auto chdir_result = Filesystem::Cwd().Chdir(build_working_dir);
  TINYSWIFT_CHECK(chdir_result.ok(),
...
#endif  // TINYSWIFT_COMMON_BAZEL_WORKING_DIR_H_
