...
#ifndef TINYSWIFT_COMMON_FILESYSTEM_H_
#define TINYSWIFT_COMMON_FILESYSTEM_H_
...
// Provides a filesystem library for use in the TinySwift project.
//
...
class DirRef::Iterator
 ...
 public:
  ...
  auto operator==(const Iterator& rhs) const -> bool {
    TINYSWIFT_DCHECK(dirp_ == nullptr || rhs.dirp_ == nullptr ||
                  dirp_ == rhs.dirp_);
...
inline auto DirRef::ReadEntries()
  ...
  TINYSWIFT_RETURN_IF_ERROR(AppendEntriesIf(entries));
  return entries;
}
...
inline auto DirRef::Chdir(const std::filesystem::path& path)
  ...
  TINYSWIFT_ASSIGN_OR_RETURN(Dir d, OpenDir(path));
  auto result = d.Chdir();
  if (result.ok()) {
    return Success();
  }
...
inline auto Dir::Iterator::operator++() -> Iterator& {
  TINYSWIFT_CHECK(dirp_, "Cannot increment an end-iterator");
  ...
  // filesystem.
  TINYSWIFT_CHECK(entry_.dent_ != nullptr || errno == 0,
...
#endif  // TINYSWIFT_COMMON_FILESYSTEM_H_
