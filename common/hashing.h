...
#ifndef TINYSWIFT_COMMON_HASHING_H_
#define TINYSWIFT_COMMON_HASHING_H_
...
// A 64-bit hash code produced by `TinySwift::HashValue`.
//
...
// `TinySwiftHashValue` with the following signature:
//
// ```cpp
// auto TinySwiftHashValue(const YourType& value, uint64_t seed) -> HashCode;
// ```
//
...
// `TinySwiftHashValue` that will in turn be used by the `HashValue` function. See
...
// Example usage:
// ```cpp
// auto TinySwiftHashValue(const MyType& value, uint64_t seed) -> HashCode {
//   Hasher hasher(seed);
...
// members that may be used by authors of `TinySwiftHashValue` implementations to
...
class Hasher {
 public:
  ...
  static auto SampleRandomData(ssize_t offset) -> uint64_t {
    TINYSWIFT_DCHECK(offset + sizeof(uint64_t) < sizeof(StaticRandomData));
    uint64_t data;
    memcpy(&data,
  ...
  static auto SampleAlignedRandomData(ssize_t offset) -> uint64_t {
    TINYSWIFT_DCHECK(static_cast<size_t>(offset) <
...
// A dedicated namespace for `TinySwiftHashValue` overloads that are not found by
...
template <typename T>
inline auto TinySwiftHashValue(llvm::ArrayRef<T> values, uint64_t seed)
    -> HashCode {
  Hasher hasher(seed);
...
inline auto TinySwiftHashValue(llvm::ArrayRef<std::byte> bytes, uint64_t seed)
    -> HashCode {
  Hasher hasher(seed);
...
inline auto TinySwiftHashValue(llvm::StringRef value, uint64_t seed) -> HashCode {
  return TinySwiftHashValue(
      llvm::ArrayRef(reinterpret_cast<const std::byte*>(value.data()),
                     value.size()),
      seed);
}

inline auto TinySwiftHashValue(std::string_view value, uint64_t seed) -> HashCode {
  return TinySwiftHashValue(llvm::StringRef(value.data(), value.size()), seed);
}

inline auto TinySwiftHashValue(const std::string& value, uint64_t seed)
    -> HashCode {
  return TinySwiftHashValue(llvm::StringRef(value.data(), value.size()), seed);
}

template <unsigned Length>
inline auto TinySwiftHashValue(const llvm::SmallString<Length>& value,
...
// element.
template <typename T>
inline auto TinySwiftHashValue(const std::vector<T>& arg, uint64_t seed)
    -> HashCode {
  return TinySwiftHashValue(llvm::ArrayRef(arg), seed);
}
template <typename T>
inline auto TinySwiftHashValue(const llvm::SmallVectorImpl<T>& arg, uint64_t seed)
    -> HashCode {
  return TinySwiftHashValue(llvm::ArrayRef(arg), seed);
}
template <typename T, size_t N>
inline auto TinySwiftHashValue(const std::array<T, N>& arg, uint64_t seed)
    -> HashCode {
  return TinySwiftHashValue(llvm::ArrayRef(arg), seed);
}
template <typename T, size_t N>
inline auto TinySwiftHashValue(const T (&arg)[N], uint64_t seed) -> HashCode {
  return TinySwiftHashValue(llvm::ArrayRef(arg), seed);
}

inline auto TinySwiftHashValue(llvm::APInt value, uint64_t seed) -> HashCode {
  Hasher hasher(seed);
...
inline auto TinySwiftHashValue(llvm::APFloat value, uint64_t seed) -> HashCode {
  Hasher hasher(seed);
...
template <typename... Ts>
inline auto TinySwiftHashValue(const std::tuple<Ts...>& value, uint64_t seed)
    -> HashCode {
  Hasher hasher(seed);
...
inline auto TinySwiftHashValue(const std::pair<T, U>& value, uint64_t seed)
    -> HashCode {
  Hasher hasher(seed);
...
// Implementation detail predicate to detect if there is a `TinySwiftHashValue`
...
template <typename T>
concept HasTinySwiftHashValue = requires(const T& value, uint64_t seed) {
  { TinySwiftHashValue(value, seed) } -> std::same_as<HashCode>;
};
...
// Implementation of the unqualified dispatch to any provided `TinySwiftHashValue`
// overloads, either here, or via ADL. Note that similar to
// `HasTinySwiftHashValue`, this must not be moved above any of those overloads.
template <typename T>
inline auto DispatchImpl(const T& value, uint64_t seed) -> HashCode {
  // If we have an explicit overload for `TinySwiftHashValue`, call it. This may be
  // provided above or via ADL, and is preferred as it represents an explicit
  // request for how the type is hashed.
  if constexpr (HasTinySwiftHashValue<T>) {
    return TinySwiftHashValue(value, seed);
    ...
    // candidates.
    static_assert(HasTinySwiftHashValue<T>,
                  "Attempted to hash a type which does not have a "
                  "`TinySwiftHashValue` overload.");
  }
}
...
// Building with `-DTINYSWIFT_MCA_MARKERS` will enable `llvm-mca` annotations in
...
#if TINYSWIFT_MCA_MARKERS
#define TINYSWIFT_MCA_BEGIN(NAME) \
  __asm volatile("# LLVM-MCA-BEGIN " NAME "" ::: "memory");
#define TINYSWIFT_MCA_END(NAME) \
...
inline auto Hasher::HashSizedBytes(llvm::ArrayRef<std::byte> bytes) -> void {
  ...
  if (size <= 8) {
    if (size >= 4) {
      TINYSWIFT_MCA_BEGIN("dynamic-8b");
    ...
    TINYSWIFT_MCA_BEGIN("dynamic-4b");
    if (size == 0) {
      buffer ^= StaticRandomData[0];
    } else {
    ...
    TINYSWIFT_MCA_END("dynamic-4b");
    return;
  }

  if (size <= 16) {
    TINYSWIFT_MCA_BEGIN("dynamic-16b");
    ...
    TINYSWIFT_MCA_END("dynamic-16b");
    return;
  }

  if (size <= 32) {
    TINYSWIFT_MCA_BEGIN("dynamic-32b");
...
#endif  // TINYSWIFT_COMMON_HASHING_H_
