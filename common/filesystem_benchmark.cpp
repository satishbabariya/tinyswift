...
namespace TinySwift::Filesystem {
namespace {
...
static auto GetText(int length) -> std::string {
  ...
  TINYSWIFT_CHECK(static_cast<int>(content.size()) == length);
  return content;
}
...
struct BenchContext {
  ...
  BenchContext() : tmpdir(*MakeTmpDir()) {
    for (int i : llvm::seq(NumFiles)) {
      ...
      TINYSWIFT_CHECK(result.ok(), "{0}", result.error());
  ...
  auto CreateTree(std::filesystem::path base, int entries, int entries_per_dir)
      -> void {
    TINYSWIFT_CHECK(entries >= 1);
    TINYSWIFT_CHECK(entries_per_dir >= 1);
    ...
    TINYSWIFT_CHECK(d.ok(), "{0}", d.error());
    ...
    while (!dir_stack.empty()) {
      ...
      TINYSWIFT_CHECK(entries_per_subdir < num_entries);
      ...
      if (entries_per_subdir >= entries_per_dir && subdir_count < num_subdirs) {
        ...
        TINYSWIFT_CHECK(subdir.ok(), "{0}", subdir.error());
        ++subdir_count;
  ...
  while (state.KeepRunningBatch(NumFiles)) {
    for (int i : llvm::seq(NumFiles)) {
      if constexpr (Comp == TinySwift) {
        ...
        TINYSWIFT_CHECK(!ec, "{0}", ec.message());
        TINYSWIFT_CHECK(exists);
      } else {
...
BENCHMARK(BM_Access<TinySwift>)->UseRealTime();
  ...
  while (state.KeepRunningBatch(NumFiles)) {
    for (int i : llvm::seq(NumFiles)) {
      if constexpr (Comp == TinySwift) {
        ...
        TINYSWIFT_CHECK(result.error().no_entity());
      } else if constexpr (Comp == Std) {
        std::error_code ec;
...
BENCHMARK(BM_AccessMissing<TinySwift>)->UseRealTime();
  ...
  while (state.KeepRunningBatch(NumFiles)) {
    for (int i : llvm::seq(NumFiles)) {
      if constexpr (Comp == TinySwift) {
        ...
        TINYSWIFT_CHECK(status.ok(), "{0}", status.error());
...
BENCHMARK(BM_Stat<TinySwift>)->UseRealTime();
  ...
  while (state.KeepRunningBatch(NumFiles)) {
    for (int i : llvm::seq(NumFiles)) {
      if constexpr (Comp == TinySwift) {
        ...
        TINYSWIFT_CHECK(status.error().no_entity());
      } else if constexpr (Comp == Std) {
        std::error_code ec;
        auto status = std::filesystem::status(
            context.tmpdir.path() / context.missing_paths[i], ec);
        TINYSWIFT_CHECK(ec.value() == ENOENT, "{0}", ec.message());
      } else {
        static_assert(false, "Invalid benchmark comparable");
      }
    }
...
BENCHMARK(BM_StatMissing<TinySwift>)->UseRealTime();
  ...
  while (state.KeepRunningBatch(NumFiles)) {
    for (int i : llvm::seq(NumFiles)) {
      if constexpr (Comp == TinySwift) {
        ...
        TINYSWIFT_CHECK(f.error().no_entity());
      } else if constexpr (Comp == Std) {
        std::ifstream f(context.tmpdir.path() / context.missing_paths[i]);
        TINYSWIFT_CHECK(!f.is_open());
      } else {
        static_assert(false, "Invalid benchmark comparable");
      }
    }
...
BENCHMARK(BM_OpenMissing<TinySwift>)->UseRealTime();
...
BENCHMARK(BM_OpenClose<TinySwift>)->UseRealTime();
  ...
  while (state.KeepRunningBatch(NumFiles)) {
    for (int i : llvm::seq(NumFiles)) {
      if constexpr (Comp == TinySwift) {
        ...
        TINYSWIFT_CHECK(remove_result.ok(), "{0}", remove_result.error());
      } else if constexpr (Comp == Std) {
...
BENCHMARK(BM_CreateRemove<TinySwift>)->UseRealTime();
  ...
  while (state.KeepRunningBatch(NumFiles)) {
    ...
    for (int i : llvm::seq(NumFiles)) {
      if constexpr (Comp == TinySwift) {
        ...
        TINYSWIFT_CHECK(read_result.ok(), "{0}", read_result.error());
...
BENCHMARK(BM_Read<TinySwift>)->Range(4, 1024LL * 1024)->UseRealTime();
  ...
  while (state.KeepRunningBatch(NumFiles)) {
    ...
    for (int i : llvm::seq(NumFiles)) {
      if constexpr (Comp == TinySwift) {
        auto write_result =
            context.tmpdir.WriteFileFromString(context.file_paths[i], content);
        TINYSWIFT_CHECK(write_result.ok(), "{0}", write_result.error());
      } else if constexpr (Comp == Std) {
...
BENCHMARK(BM_Write<TinySwift>)->Range(4, 1024LL * 1024)->UseRealTime();
  ...
  while (state.KeepRunningBatch(batch_size)) {
    ...
    for (int i : llvm::seq(batch_size)) {
      ...
      if constexpr (Comp == TinySwift) {
        ...
        TINYSWIFT_CHECK(rmdir_result.ok(), "{0}", rmdir_result.error());
      } else if constexpr (Comp == Std) {
        std::error_code ec;
...
auto BM_CreateDirectories(benchmark::State& state) -> void {
  ...
  TINYSWIFT_CHECK(existing_depth <= depth);
  TINYSWIFT_CHECK(depth > 0);
  ...
  while (state.KeepRunningBatch(BatchSize)) {
    ...
    for (int i : llvm::seq(BatchSize)) {
      if constexpr (Comp == TinySwift) {
        ...
        TINYSWIFT_CHECK(result.ok(), "Failed to create '{0}': {1}", paths[i],
                     result.error());
...
BENCHMARK(BM_CreateDirectories<TinySwift>)
