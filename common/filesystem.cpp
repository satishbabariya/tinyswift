...
auto Internal::FileRefBase::ReadFileToString()
  ...
  TINYSWIFT_RETURN_IF_ERROR(SeekFromBeginning(0));
  for (;;) {
...
auto Internal::FileRefBase::WriteFileFromString(llvm::StringRef str)
  ...
  TINYSWIFT_RETURN_IF_ERROR(Truncate(str.size()));
  return Success();
}
...
static auto SleepMacos(Duration sleep) -> void {
  ...
  for (;;) {
    ...
    if (errno != EINTR) {
      ...
      TINYSWIFT_FATAL("Unexpected error while sleeping: {0}", error_os.TakeStr());
    }
...
static auto Sleep(Duration sleep) -> void {
  ...
  TINYSWIFT_CHECK(result == 0, "Error getting the time: {0}", strerror(errno));
...
auto Internal::FileRefBase::TryLock(FileLock::Kind kind, Duration deadline,
  ...
  TINYSWIFT_CHECK(poll_interval <= deadline);
  ...
  if (deadline != Duration(0)) {
    ...
    TINYSWIFT_CHECK(poll_interval >= std::chrono::microseconds(1),
...
auto DirRef::AppendEntriesIf(
    ...
    -> ErrorOr<Success, FdError> {
  TINYSWIFT_ASSIGN_OR_RETURN(Reader reader, Read());
...
auto DirRef::OpenDir(const std::filesystem::path& path,
  ...
  if (creation_options != OpenExisting) {
    TINYSWIFT_CHECK(creation_options != CreateAlways,
...
auto DirRef::ReadFileToString(const std::filesystem::path& path)
  ...
  TINYSWIFT_ASSIGN_OR_RETURN(ReadFile f, OpenReadOnly(path));
...
auto DirRef::WriteFileFromString(const std::filesystem::path& path,
  ...
  TINYSWIFT_ASSIGN_OR_RETURN(WriteFile f, OpenWriteOnly(path, creation_options));
...
auto DirRef::CreateDirectories(const std::filesystem::path& path,
  ...
  TINYSWIFT_CHECK(!missing_components.empty());
...
auto DirRef::Rmtree(const std::filesystem::path& path)
  ...
  for (;;) {
    ...
    TINYSWIFT_CHECK(dir_entry_start < static_cast<ssize_t>(dir_entries.size()));
    ...
    if (rmdir_result.ok() || rmdir_result.error().no_entity()) {
      ...
      if (dir_entries.empty()) {
        ...
        TINYSWIFT_CHECK(dir_stack.empty());
        return Success();
      }
      continue;
    }
    ...
    TINYSWIFT_ASSIGN_OR_RETURN(Dir subdir, current.OpenDir(entry_path));
...
auto DirRef::ReadlinkSlow(const std::filesystem::path& path)
  ...
  TINYSWIFT_ASSIGN_OR_RETURN(FileStatus status, Lstat(path));
...
auto MakeTmpDirWithPrefix(std::filesystem::path prefix)
  ...
  TINYSWIFT_CHECK(result == tmpdir_path_buffer.data(),
  ...
  TINYSWIFT_ASSIGN_OR_RETURN(FileStatus stat, result_dir.Stat());
